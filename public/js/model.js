                ...
                success: function (responseStr) {
                    ...
                    if ((checkval == 'Y')) {
                        ...
                        var params = {
                            ...
                            'sSQL' : response.sql,
                            'scored_file_name' : response.list_short_name,
...
function loadList(resStr, up_flag) {
    ...
    var scored_file_name = responseStr.Scored_File_Name;
    var Report_Fields = JSON.parse(responseStr.Report_Fields);
    var ModelScore_Des = responseStr.modelscoremetadata.ModelScore_Des;
    var ModelScore_Universe = responseStr.modelscoremetadata.ModelScore_Universe;
    var ModelScore_Cutoff_Date = responseStr.modelscoremetadata.ModelScore_Cutoff_Date;
    var list_level = responseStr.list_level;

    $('#modelbuildid').val(responseStr.ModelBuildID).trigger('chosen:updated');

    $('#list_level').val(list_level);

    $("#accordion").html('');
    $("#accordion").html(resStr.fieldsHtml);

    $("#collapseFields").html('');
    $("#collapseFields").html(resStr.fieldscollapsehHtml);
    ...
    $('#scored_file_name').val(scored_file_name);
    $('#scored_file_description').val(ModelScore_Des);
    $('#scored_file_universe').val(ModelScore_Universe);
    $('#scored_file_cuttoffdate').val(ModelScore_Cutoff_Date);

    $('#fields').val(Report_Fields);

    $('#fields').multiselect('refresh');

    setTimeout(function () {
        $('[id="fields_ms"]').attr('style',
            'width:100% !important;' +
            'height: 28px; ' +
    ...
    /*$.each(Report_Fields, function (index,value) {
...
function session(runoption) {
    ...
    var scored_file_name = document.getElementById('scored_file_name').value;
    ...
    var modelbuildid = $('#modelbuildid').val()
    var scored_file_description = $('#scored_file_description').val()
    var scored_file_universe = $('#scored_file_universe').val()
    var scored_file_cuttoffdate = $('#scored_file_cuttoffdate').val()
    ...
    var report_fields = $.trim($('#fieldReportSummaryVal').text());
    ...
    var params = {
        ...
        'scored_file_name' : scored_file_name,
        ...
        'scored_file_description' : scored_file_description,
        'scored_file_universe' : scored_file_universe,
        'scored_file_cuttoffdate' : scored_file_cuttoffdate,
        'cI' : cI,
    ...
    parent.Camp_Name =scored_file_name;
    parent.document.getElementById('list_level').value = list_level;
    parent.document.getElementById('scored_file_description').value = scored_file_description;
    ...
    var record = {
        camp_id : campid,
        Camp_Name : list_name,
        list_short_name : scored_file_name,
        meta_description : scored_file_description,
        sSQL : sSQL,
        list_level : list_level,
        selected_fields : selected_fields,
        ...
        success: function (response) {
            ...
            if (response !== undefined) {
                if(response.count > 0){
                    ...
                    var expoCol = 'ModelBuildID:false|SegmentID:true|ModelScore:true|ModelDecile:true|ModelTier:true|';
                    ...
                    parm.eExt = "xlsx";
                    parm.eFile = parm.scored_file_name;
                    parm.eFolder = "Public";
                    parm.saveCD = "Y";
                    parm.saveFile = "Y";
...
function show_Create_library(obj) { // Change Campaign - 2014-03  begin
    ...
    if (libflag_name == 'email') {
        ...
        $.ajax({
            ...
            success: function (datao) {
                ...
                $('#txtSub1').val($('#clientname').val() + ' - Model ' + dataArr.list_short_name);
                $('#emailBox').modal('show');
            }
        });
        obj.val(0);
    ...
    if (libflag_name == 'share') {
        ...
        obj.val(0);
        $('#chkAddToPhone').parents('.form-group').hide();
        $.ajax({
            type: 'GET',
            url: 'getshare',
            data: postData,
        ...
        $.ajax({
            ...
            success: function (datao) {
                ...
                params = {
                    ...
                    scored_file_name : dataouter.list_short_name,
                    meta_description : dataouter.modelscoremetadata.ModelScore_Des,
                    sSQL : dataouter.sql,
                    list_level : dataouter.list_level,
                    selected_fields : dataouter.selected_fields,
                    CampaignID : dataouter.t_id,
                    Type : 'M',
                    report_fields : dataouter.Report_Fields,
                ...
                document.frmExec.action = "model/reschedule";
...
function get_distribution_show(recid) {
    ...
    $.ajax({
        ...
        success: function (datao) {
            ...
            var scored_file_name = dataouter.list_short_name;
            /*var metaData = dataouter.meta_data;
            var metaStr = metaData.split('^');*/
...
function listShortname() {
    var string = document.getElementById("scored_file_name").value;
    if (string.length > 20) {
        $('#indicationMsg').fadeIn('slow');
        $('#indicationMsg').text('Scored File Name should be less than 20 characters');
        ...
        $("#scored_file_name").val($.trim(string.replace(/[`~!@#$%^&*()|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, '')));
        $('#indicationMsg').fadeIn('slow');
...
function changeListShortName() {
    ...
    var scored_file_name = $('#scored_file_name').val();
    /*if (scored_file_name == "") {
            $('#indicationMsg').fadeIn('slow');
    ...
    //var lListName = pPreFix + "_" + scored_file_name + "_" + dDate;
    var lListName = scored_file_name + "_" + dDate;
...
function add_field(fieldname,action) {
    ...
    if (lListFields == "custom_fields") {
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#reportaccordion").html('');
                $("#reportaccordion").html(data.fieldsHtml);

                $("#collapseFields").html('');
                $("#collapseFields").html(data.fieldscollapsehHtml);

                $("#collapseReportFields").html('');
                $("#collapseReportFields").html(data.fieldscollapsehHtml);
                ...
                initJS($('#reportaccordion'));
                initJS($('#collapseReportFields'));
...
function clearFilterSummary() {
    ...
    $(':input', '#accordion','#reportaccordion').not(':button, :submit, :reset, :hidden').removeAttr('checked').removeAttr('selected');
...
function addsubSQL(runoption = true) {
    if ($.trim($('#modelbuildid').val()) == "") {
        $('#indicationMBIMsg').fadeIn('slow');
        $('#indicationMBIMsg').text('Please select the model');
        $('#indicationMBIMsg').attr('style', 'color:red;');
        setTimeout(function () {
            $('#indicationMBIMsg').fadeOut(2000);
            $('#indicationMBIMsg').text('');
        }, 3000);
        return false;
    }

    if ($.trim($('#scored_file_name').val()) == "") {
        $('#indicationMsg').fadeIn('slow');
        $('#indicationMsg').text('Please enter scored file name');
        $('#indicationMsg').attr('style', 'color:red;');
        setTimeout(function () {
            $('#indicationMsg').fadeOut(2000);
            $('#indicationMsg').text('');
    ...
    if(runoption == true){
        $('#saveoption').html('<span class="spinner-border spinner-border-sm ds-c" role="status" aria-hidden="true"></span><span class="sr-only">Loading...</span>');
    }
    ...
    if (up_flag == 'new') {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (flag == 1) {
                    var data = {
                        'title': 'Scored File Name Exists.... Please enter new campaign name ',
                        'text' : '',
                        'butttontext' : 'Ok',
                        'cbutttonflag' : false
                    };
                    ...
                    $('#savebottom').attr('disabled', false).html('').text('Run Now');
                    return false;
                } else {
                    document.getElementById('is_name_exist').value = '';
                    Camp_Name = document.getElementById('list_name').value;
...
function downloadFile50k(fileType) {
    ...
    var fFileName = $.trim($('#scored_file_name').val()) != "" ? $.trim($('#list_name').val()) : 'ReportWith10K' ;
...
function delete_row_comp(libcamp_id) {
    ...
    if (ans) {
        $.ajax({
            type: 'POST',
            url: 'delete',
            data: {del_row : libcamp_id , type : 'M', _token : $('[name="_token"]').val()},
            async: false,
...
ACFn.ajax_run_report_result = function (F,R) {
    if (R.success) {
        ...
        if(rowVariable != ""){
            ...
            if($('#scored_file_name').val() == ""){
                var rpName = stringSanitize(rpheader, {'and' : '','Segment' : '', '_' : '','Lifetime': 'Life','Amount' : ''},20)
                $('#scored_file_name').val(rpName);
                $('#scored_file_name').trigger('keyup');
            }
        }
...
function changemodel(obj) {
    var modelbuild = obj.find('option:selected').text().split(' - ');
    $('#scored_file_description').val('');
    $('#scored_file_description').val(modelbuild[1] + ' ' + modelbuild[2] + ' Scored File ' + $('#scored_file_description').val())
}

function generateXLSX(row_id, obj) {
    obj.html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true" style="color: #6093b2;"></span>');

    setTimeout(function () {
        ACFn.sendAjax('model/generateXLSX','post',{
            row_id : row_id,
        },obj,{},false);
    },1000);
}

function generatePDF(row_id, obj) {
    obj.html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true" style="color: #6093b2;"></span>');

    setTimeout(function () {
        ACFn.sendAjax('model/generatereport','post',{
            row_id : row_id,
        },obj,{},false);
    },1000);
}
...
ACFn.ajax_generate_report = function (F,R) {
    if (R.success) {
        var pdfdata = [];
        ...
        function drawChart() {
            ...
            $.each(R.response, function (index,item) {
                var dData = [];
                dData.push([item.row_variable.replace(/_+/g, ' '), 'Universe', {role : 'style'}, 'Predicted Response']);
                $.each(item.data, function (idx, im) {
                    if (im.Value != 'Total')
                        dData.push([im.Value, parseInt(im.Universe),'stroke-color: #96d7fd; stroke-width: 2; fill-color: #cdeefd', parseFloat(im.PredictedResponse)])
                });
                ...
                if(index === 0){
                    ctitle = item.row_variable.replace(/[\[\]']+/g,'').replace(/_+/g, ' ');
                    htitle = '';
                    //legend = 'none';
                }
                ...
                var options2 = {
                    ...
                    axes: {
                        y: {
                            ...
                            PredictedResponse : {label: 'Predicted Response'}
                        }
                    },
                    'is3D': true,
                ...
                google.visualization.events.addListener(chart, 'ready', function () {
                    setTimeout(function () {
                        var entry = {
                            ...
                            chart_title : item.variable_type +' - '+ item.row_variable.replace(/[\[\]']+/g,'').replace(/_+/g, ' '),
            ...
            setTimeout(function () {
                $('.chart').html('');
                ACFn.sendAjax('model/generatePDF','POST', {
                    pdfdata : localStorage.getItem("pdfdata"),
                    rpheader : R.record.modelscoremetadata.ModelScore_Des,
                    rpfooter : R.record.Scored_File_Name,
                    filename : R['record']['rpschedule']['moschstatusmap'][0]['file_name'],
                    folder_location : R.record.promoexpo_folder,
                    papersize : 'portrait'
                },F,{ progress: 'none'});
            },2000)
...
function d_pdf(is_download) {
    ...
    if (is_download == 1) {
        ...
        var $html = '';
        var rpfooter = $.trim($('#scored_file_name').val()) != "" ? $('#scored_file_name').val() : srData.list_short_name
    ...
    if(is_download == 2){
        ...
        var $html = '';
        var rpfooter = $.trim($('#scored_file_name').val()) != "" ? $('#scored_file_name').val() : srData.list_short_name
        if($('#scored_file_name').length > 0 && $('#scored_file_name').val() == ""){
            var rpheader = $.trim($('#scored_file_name').val());
        }else if($('#scored_file_name').length == 0){
...
ACFn.ajax_download_sr_file = function (F,R) {
    ...
    if(R.filetype){
        var icon = R.filetype == 'xlsx' ? '<i class="fas fa-file-excel" style="color: #06b489;"></i>' : '<i class="fas fa-file-pdf" style="color: #e92639;"></i>';
        F.html(icon);
        F.attr('href',R.filepath).removeAttr('onClick');
    }
...
function tagcampaign(obj,rowId,actionType) {
...
function downloadESDLink(obj){
    var url = obj.data('href');
    var tab = obj.attr('data-tab');
    var modelscoreid = $('#modelscoreid').length ? $('#modelscoreid').val() : '';
    if(tab == 'SingleCamp'){
        if(modelscoreid == ''){
            $('#modelscoreid').parents('.form-group').addClass('has-error');
            $('#modelscoreid').after('<span class="error-block help-block">Campaign is required</span>');
        ...
        var table = $('#singlecampevals').find('table').html()
        ACFn.sendAjax(url,'POST',{
            tab : tab,
            table : '<table style="width: 50%;" class="table table-bordered table-hover color-table lkp-table">' + table + '</table>',
        ...
        ACFn.sendAjax(url,'GET',{
            ...
            singlecampaignid : modelscoreid
        },obj);
    }

}

function ajax_quick_update(obj) {
    var rowID = obj.data('rowid');
    var fieldname = obj.data('field');
    var fieldvalue = obj.val();
    delay(function(){
        ACFn.sendAjax('campaign/metadataquickupdate','GET',{
            rowID : rowID,
            fieldname : fieldname,
