...
function save() {
    ...
    if (libview.checked == false && libupdate.checked == false && libnew.checked == false) { //console.log('if');
        ...
        if ($.trim($('#list_name').val()) == '') {
            var data = {
                'title': 'Please Enter New Report Name ',
                'text' : '',
                'butttontext' : 'Ok',
                'cbutttonflag' : false
            };
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (flag == 1) {
                        ...
                        var data = {
                            'title': 'Report Name already exists....Please Enter New Report Name',
                            'text' : '',
                            'butttontext' : 'Ok',
                            'cbutttonflag' : false
                        };
...
function session(runoption = true) {
    $('#save','#savenow').attr('disabled', true);
...
function show_Create_library(obj) { // Change Campaign - 2014-03  begin
    ...
    if (libflag_name == 'email') {
        ...
        $('#set_type').val('A');
        $('#sendemail-title').text('Send Report via Email');
        ...
        $.ajax({
            ...
            success: function (datao) {
                NProgress.done(true);
                var dataArr = datao.aData;
                $('#txtSub1').val($('#clientname').val() + ' - Report ' + dataArr.list_short_name);
                $('#emailBox').modal('show');
            }
        });
        obj.val(0);
    ...
    console.log('libflag_name--',libflag_name,'--');
...
function addsubSQL(runoption = true) {
    if ($.trim($('#listShortName').val()) == "") {
        $('#indicationMsg').fadeIn('slow');
        $('#indicationMsg').text('Please enter report name');
        $('#indicationMsg').attr('style', 'color:red;');
        setTimeout(function () {
            $('#indicationMsg').fadeOut(2000);
            $('#indicationMsg').text('');
    ...
    if (up_flag == 'new') {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (flag == 1) {
                    var data = {
                        'title': 'Report Name Exists.... Please Enter New Report Name ',
                        'text' : '',
                        'butttontext' : 'Ok',
                        'cbutttonflag' : false
                    };
    ...
    $('.list_Define_img').hide();
    $('.list_Library').hide();
    var sSQL = document.getElementById('sqlQuery').value;
    if (sSQL != '') {
        seg_openFlag = 'Y';
        if (addsubgroupchk == 'N') {
            $('#save','#savenow').attr('disabled', true);
