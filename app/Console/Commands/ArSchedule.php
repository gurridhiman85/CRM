...
class ArSchedule extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'arSchedule:run {sid}';
    ...
    public function handle()
    {
        ...
        $SQL = DB::select("Select [ftp_tmpl_id],[camp_tmpl_id],[Schedule_Name],[sch_status_id],[Schedule_type],[rp_count],[ftp_tmpl_id]
               ...
               ,[rp_days],[rp_end_date],[rp_run_time],[metadata_date],[SFTP_Attachment] from [UL_RepCmp_Schedules] Where row_id = '$sch_id' AND t_type = 'A'");
...
//  $CGD = $aData[0]['asg_camp_dtls'];
        ...
        if ($SchType == 'RP') {
            ...
            if ($rp_count > 1) {
                ...
                $cid1 = $cid + 1;
                DB::update("UPDATE [UR_Report_Sequence] SET [camp_id] = " . $cid1);
            ...
            $rp_count = $rp_count + 1;
            DB::update("UPDATE [UL_RepCmp_Schedules] SET [rp_count] = $rp_count Where row_id = '$sch_id' AND t_type = 'A'");
        }
        ...
        if ((trim($file_Opt) == 'Y' && trim($sSQL) != '') || ((trim($CD_Opt) == 'Y') && trim($sSQL) != '')) {
            ...
            if ($ftp_id != 0) {
                ...
                if ($site_type == 'FTP') {
                    ...
                    if (($conn_id) && ($login_result)) {
                        ...
                        $ftp_flag = 'N';
                        DB::update("Update UL_RepCmp_Completed set [ftp_flag] = 'N' Where row_id = '" . $sch_id . "' AND t_type = 'A'");
                        $smtp_flag = 'N';
                    }
                ...
                foreach ($ToUsers as $ToUser){
                    ...
                    if($user) {
                        ...
                        if (count(Mail::failures()) > 0) {
                            ...
                            DB::update("UPDATE UL_RepCmp_Email SET Email_Status = 'Sent' WHERE camp_tmpl_id = '" . $CID . "' AND t_type='A'");
                        }
                    }
                }
            ...
            $sSqlSRE = DB::select("SELECT Shared_With_User_id FROM UL_RepCmp_Share WHERE Is_Delivered <> 1 AND camp_tmpl_id='".$CID."' AND t_type='A'");
            ...
            if($List_Format != 'default'){
                ...
                try{
                    ...
                    PDF::loadView('layouts.pdf-v2', [
                        ...
                        'selections' => ''
                    ])->setPaper('letter',$List_Format)->setWarnings(false)->save(public_path('\\' .$folder_Name.'\\'.$this->prefix.'RPL_'.$file_Name.'.pdf'));
