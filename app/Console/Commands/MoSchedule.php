...
use App\Model\ModelScoreMetadata;
use App\User;
...
class MoSchedule extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'moSchedule:run {sid}';
    ...
    public function handle()
    {
        ...
        $asql = DB::select("Select [ftp_tmpl_id],[camp_tmpl_id],[Schedule_Name],[sch_status_id],[Schedule_type],[rp_count],[ftp_tmpl_id]
               ...
               ,[rp_days],[rp_end_date],[rp_run_time],[metadata_date],[SFTP_Attachment] from [UL_RepCmp_Schedules] Where row_id = '$sch_id' AND t_type = 'M'");
        ...
        $SQLSSM = DB::select("Select * from [UL_RepCmp_Sch_status_mapping] Where sch_id = '$sch_id' AND t_type = 'M' ORDER BY row_id DESC");
        ...
        $CampSQL = DB::select("SELECT [row_id],[t_id],[ModelBuildID],[Scored_File_Name],[list_level],[t_name],[sql],[seg_def],[seg_method],[seg_criteria],[seg_selected_criteria],[seg_grp_no],[seg_ctrl_grp_opt],[seg_camp_grp_dtls]
		    ...
		    ,[promoexpo_ecg_opt],[promoexpo_data],[selected_fields],[selected_fields],[sql],[Report_Row],[Report_Column],[Report_Function],[Report_Sum],[Report_Show],[Report_Orientation],[Custom_SQL],[SR_Attachment],[List_Format],[Lookup_Type] 
		     FROM [UM_ModelScore_Templates] Where row_id = '" . $CampID . "' AND t_type='M'");
        ...
        if (!empty($aData)) {
            ...
            foreach ($aData as $k => $row) {
                ...
                $MS_row_id = $aData[0]['row_id'];
                $CID = $aData[0]['t_id'];
                $ModelBuildID = $aData[0]['ModelBuildID'];
                $Scored_File_Name = $aData[0]['Scored_File_Name'];
        ...
        $metaData = RepCmpMetaData::where('CampaignID',$CID)->where('type','M')->first();
        if($metaData){
            $metadata_date = $metaData->Start_Date;
        }
        $file_Name1 = $Scored_File_Name;
        ...
        if ($SchType == 'RP') {
            ...
            if ($rp_count > 1) {
                ...
                $cid1 = $cid + 1;
                DB::update("UPDATE [UM_ModelScore_Sequence] SET [camp_id] = " . $cid1);
            ...
            $rp_count = $rp_count + 1;
            DB::update("UPDATE [UL_RepCmp_Schedules] SET [rp_count] = $rp_count Where row_id = '$sch_id' AND t_type = 'M'");
        }
        ...
        if ((trim($file_Opt) == 'Y' && trim($sSQL) != '') || ((trim($CD_Opt) == 'Y') && trim($sSQL) != '')) {
            ...
            if (($SchType == 'RP') || ($SchType == 'RA')){
                //DB::update("UPDATE [UL_RepCmp_Status] SET [status] = 'Running', [start_time] = '" . $date . "' Where row_id = '" . $SchStatusID . "' AND t_type = 'M'");
            }
            //else
            //DB::update("UPDATE [UL_RepCmp_Status] SET [status] = 'Running' Where row_id = '" . $SchStatusID . "' AND t_type = 'M'");
            ...
            foreach ($expcolsarray as $k => $v) {
                if(!empty($v)){
                    ...
                    if (trim($kk[1]) == 'true') {
                        ...
                        if (!in_array($kk[0],['ModelScore','ModelDecile','ModelTier'])) {
            ...
            $mScoreMetaData = DB::select("SELECT ModelScoreID,ModelScore_Des FROM UM_ModelScore_Metadata WHERE ModelScoreID = '".$CID."'");
            $mScoreMetaData = collect($mScoreMetaData)->map(function($x){ return (array) $x; })->toArray();
            $Scored_File_Description = $mScoreMetaData[0]['ModelScore_Des'];
            $sampleArray = explode("^", $sample);
            $samplePercentage = explode(":", $sampleArray[0]);
            $sSper = $samplePercentage[0];
            if ($segDef == 'none' && $segFiltersCriteria == '') {
                DB::statement("alter table $tblName add ModelBuildID int null , groupid int null , segmentid int null,numgroups smallint null,ModelScoreID int null");

                DB::update("update $tblName set ModelBuildID=$ModelBuildID ,numgroups = $noCG,ModelScoreID=".$mScoreMetaData[0]['ModelScoreID']);
                ...
                DB::insert("insert into UM_ModelScore_Data (DS_MKC_Contactid,ModelScoreID, ModelBuildID, groupid, segmentid, email) select DS_MKC_ContactID,ModelScoreID, ModelBuildID, groupid, segmentid, $emailCol from $tblName");
                ...
                DB::statement("alter table $tblName add ModelBuildID int null , groupid int null , segmentid int null,numgroups smallint null,ByField nvarchar(255) null,ModelScoreID int null");
                ...
                $emailCol = isset($aDataCheck[0]['Email']) && !empty($aDataCheck[0]['Email']) ? "Email" : "'Null' as Email";
                DB::insert("insert into UM_ModelScore_Data (DS_MKC_Contactid,ModelBuildID, ModelBuildID, groupid, segmentid, email) select DS_MKC_ContactID,ModelBuildID, ModelBuildID, groupid, segmentid, $emailCol from $tblName");
            }
                ...
                DB::update("update $tblName set ModelBuildID=$ModelBuildID,numgroups=$noCG,ModelScoreID=$mScoreMetaData[0]['ModelScoreID'], SegmentID= case $cCondition else null end");
            ...
            foreach ($expcolsarray as $k => $v) {
                if(!empty($v)){
                    ...
                    if (trim($kk[1]) == 'true') {
                        $kk[0] = trim($kk[0]);
                        $kk[0] = $kk[0] == 'DS_MKC_ContactID' ? 'c.DS_MKC_ContactID' : $kk[0];
                        if ($flag != 1) {
                            $expcols = $expcols . "" . ($kk[0] == 'c.DS_MKC_ContactID' ? 'DS_MKC_ContactID' : $kk[0]);
                            $cols = $cols . "" . $kk[0];
                            $flag = 1;
                        } else{
                            $expcols = $expcols . "," . ($kk[0] == 'c.DS_MKC_ContactID' ? 'DS_MKC_ContactID' : $kk[0]);
            ...
            DB::statement("EXEC sp_CRM_ModelScore_" . $ModelBuildID . " ". $mScoreMetaData[0]['ModelScoreID']);
            $tblName = "tmp_". rand(0,100) . "_" . time() . "_" . date("Ymd", time());
            //echo "SELECT " . $cols . " into ".$tblName." FROM ".$list_level." c INNER JOIN  UM_ModelScore_Data m on c.DS_MKC_ContactID=m.DS_MKC_ContactID where modelscoreid = ".$mScoreMetaData[0]['ModelScoreID'];

            DB::statement("SELECT " . $cols . " into ".$tblName." FROM ".$list_level." c INNER JOIN  UM_ModelScore_Data m on c.DS_MKC_ContactID=m.DS_MKC_ContactID where modelscoreid = ".$mScoreMetaData[0]['ModelScoreID']);
            ...
            //Todo
            //Check Campaign ID Exists or not
            //$lastPart = explode($Scored_File_Name,$t_name);
            ...
            if (trim($file_Opt) == 'Y') {
                if(in_array($SR_Attachment,['onlylist','both'])) {
                    $fhead = fopen($this->filePath . 'public\\' . $folder_Name . "\\" . $this->prefix . "MOL" . $file_Name . "." . $file_Ext, 'a');
                    $expcols = $expcols . "\n";
            ...
            for ($i = 0; $i < $where_count; $i++) {
                ...
                if ((trim($file_Opt) == 'Y') && (trim($file_Ext) == "csv")) {
                    ...
                    DB::statement("insert into openrowset  ('Microsoft.ACE.OLEDB.12.0','Text;Database=" . $this->filePath .'public\\'. $folder_Name . "\\;HDR=YES;FMT=Delimited','SELECT $expcols FROM [".$this->prefix."MOL$file_Name.csv]' ) $newsql");
                    //var_dump($aData);
                    ...
                    if(!in_array($SR_Attachment,['onlylist','both'])){
                        unlink($this->filePath .'public\\'. $folder_Name . "\\".$this->prefix."MOL$file_Name.csv");
                    }
                    ...
                    if(in_array($SR_Attachment,['onlylist','both'])) {
                        ...
                        $writer->save(public_path() . '\\' . $folder_Name . "\\" . $this->prefix . "MOL" . $file_Name . ".xlsx");

                        DB::statement("insert into OPENROWSET('Microsoft.ACE.OLEDB.12.0', 'Excel 12.0;Database=" . $this->filePath . 'public\\' . $folder_Name . "\\" . $this->prefix . "MOL" . $file_Name . ".xlsx;','SELECT * FROM [Worksheet$]') $newsql");
                    }
            ...
            if (trim($CD_Opt) == 'Y') {
                ...
                if (($SchType == 'RI') or ($SchType == 'RA') or (($SchType == 'RP') && ($rp_count == 2))) {
                    ...
                    $nextSQL = DB::select("SELECT [last_runtime] from [UL_RepCmp_Status] Where row_id = '" . $SchStatusID . "' AND t_type = 'M'");
                ...
                DB::update("UPDATE [UL_RepCmp_Schedules] SET [metadata_date] = '$metadata_date' Where row_id = '$sch_id' AND t_type = 'M'");
                ...
                /*DB::insert("INSERT INTO [UM_ModelScore_Metadata] ([ModelScore_Name] ,[ModelScore_Des],[ModelScore_Universe],[ModelScore_Cuttoff_Date],[ModelID])
								 VALUES ('$Scored_File_Name','$Scored_File_Description','$Scored_File_Universe','$Scored_File_Cuttoff_Date','$ModelBuildID')");*/
  ...
  where d.campaignid=$CID");*/
            ...
            if ($ftp_id != 0) {
                ...
                if ($site_type == 'FTP') {
                    ...
                    if (($conn_id) && ($login_result)) {
                        $local_file = $this->filePath .'public\\' .$folder_Name . "\\MOL$file_Name." . "$file_Ext";
                        $remote_file = "MOL$file_Name." . "$file_Ext";
                        ...
                        ftp_close($conn_id);
                        $ftp_flag = 'Y';
                        DB::update("Update UL_RepCmp_Completed set [ftp_flag] = 'Y' Where row_id = '" . $sch_id . "' AND t_type = 'M'");
                        $smtp_flag = 'Y';
                        ...
                        $ftp_flag = 'N';
                        DB::update("Update UL_RepCmp_Completed set [ftp_flag] = 'N' Where row_id = '" . $sch_id . "' AND t_type = 'M'");
                        $smtp_flag = 'N';
                    }
                    ...
                    if(in_array($SFTP_Attachment,['onlylist','both'])){
                        $line1 = "put ".$this->prefix."MOL" . $file_Name . "." . $file_Ext . "\n";
                        $line3 = "mv ".$this->prefix."MOL" . $file_Name . "." . $file_Ext . " $folder\n ";
                    }
            ...
            if ($SchType == 'RA') {
                ...
                DB::update("UPDATE [UL_RepCmp_Status] SET [status] = 'Completed',[completed_time] = '" . $date . "' ,[succ_flag] = 'Y',[file_name] = '" . $file_Name . "',total_records= '$CSVTotalRec' Where row_id = '" . $SchStatusID . "' AND t_type = 'M'");
            } else if ($SchType == 'RP') {
                $nextSQL = DB::select("SELECT * from [UL_RepCmp_Status] Where row_id = '" . $SchStatusID . "' AND t_type = 'M'");
                $aData = collect($nextSQL)->map(function($x){ return (array) $x; })->toArray();
                ...
                if (date("Y-m-d", strtotime($next_runtime)) < date("Y-m-d", strtotime($rp_end_date))){
                    DB::insert("INSERT INTO [UL_RepCmp_Status]([sche_name],[templ_name],[start_time]
                                   ...
                                   VALUES ('$sche_name','$templ_name','$start_time','','-','','Scheduled','" . $last_runtime . "','" . $next_runtime . "','".$file_path."','$ftp_flag','M')");

                    $SQL = DB::select("Select [row_id] from [UL_RepCmp_Status] Where sche_name = '$sche_name' AND t_type = 'M' order by row_id desc");
                ...
                //Helper::generateSrPDF($rv,$cv,$fu,$sv,$sa,$sSQL,$list_level,$Scored_File_Name,$imgTag,$imgPath,$file_path,$file_Name,$this->prefix  . 'MOR_',$SR_Attachment,$Scored_File_Description,$Report_Orientation);
                ...
                /*if (date("Y-m-d", strtotime($next_runtime)) < date("Y-m-d", strtotime($rp_end_date)))
                    ...
                    $updateStatusSQL = "UPDATE [UL_RepCmp_Status] SET [status] = 'Completed',[last_runtime] = '" . $last_runtime . "',[completed_time] = '" . $date . "',[next_runtime] = '" . $next_runtime . "',[succ_flag] = 'Y',[file_name] = '" . $file_Name . "' Where row_id = '" . $SchStatusID . "' AND t_type = 'M'";
            ...
            DB::insert("Insert into [UL_RepCmp_Completed]([sche_name],[templ_name],[start_time],[next_runtime],[completed_time]
		     ...
		     ,'Completed',[file_path],'$CID','$CSVTotalRec',t_type FROM [UL_RepCmp_Status] Where row_id = '" . $SchStatusID . "' AND t_type = 'M' ORDER BY row_id DESC");



            $record = ModelScoreTemplate::with(['momodel','modelscoremetadata'])->where('row_id',$MS_row_id)->first()->toArray();
            $Report_Fields = json_decode($record['Report_Fields'], true);
            $model_variables = explode(',', $record['momodel']['Model_Variables']);
            //$variables = array_merge(['Model Tier','Model Decile'],$Report_Fields);
            $variables = array_merge($model_variables,$Report_Fields);
            ...
            DB::statement("EXEC [dbo].[sp_CRM_ModelScore1] ".$modelScoreID);
            //$stmt = $db->prepare("EXEC [dbo].[sp_CRM_ModelScore1] ".$modelScoreID);
            //$stmt->execute();

            foreach ($variables as $variable){
                DB::statement("EXEC [dbo].[sp_CRM_ModelScore2] " . $modelScoreID . ", '" . trim($variable) . "'");
                //$stmt = $db->prepare("EXEC [dbo].[sp_CRM_ModelScore2] " . $modelScoreID . ", '" . trim($variable) . "'");
                //$stmt->execute();
            }

            DB::statement("EXEC [dbo].[sp_CRM_ModelScore3] " . $modelScoreID);
            //$stmt = $db->prepare("EXEC [dbo].[sp_CRM_ModelScore3] " . $modelScoreID);
            //$stmt->execute();
        ...
        /*if($List_Format != 'none'){
            ...
            $header = ucfirst($Scored_File_Description);
            $footer = ucfirst($Scored_File_Name); //ucfirst('test');
            try{
                ...
                PDF::loadView('layouts.pdf-v2', [
                    ...
                    'selections' => ''
                ])->setPaper('letter',$List_Format)->setWarnings(false)->save(public_path($folder_Name.'\\'.$this->prefix.'MOL'.$file_Name.'.pdf'));
        ...
        $sSqlSRE = DB::select("SELECT * FROM UL_RepCmp_Email WHERE camp_tmpl_id = '" . $CID . "' AND t_type='M'");
        ...
        if (!empty($result)) {
            ...
            foreach ($ToUsers as $ToUser){
                ...
                if($user) {
                    ...
                    $objDemo->listShortName = $Scored_File_Name;
                    ...
                    if (count(Mail::failures()) > 0) {
                        ...
                        DB::update("UPDATE UL_RepCmp_Email SET Email_Status = 'Sent' WHERE camp_tmpl_id = '" . $CID . "' AND t_type='M'");
                    }
                }
            }
        ...
        $sSqlSRE = DB::select("SELECT Shared_With_User_id FROM UL_RepCmp_Share WHERE Is_Delivered <> 1 AND camp_tmpl_id='".$CID."' AND t_type='M'");
        ...
        if(isset($records) && count($records) > 0){
            foreach ($records as $record){
                ...
                if($user){
                    Helper::sendShareEmail($record,$user,$this->clientname,$record['Custom_Message'],$Scored_File_Name,'M');
                }
            }
        }
    }
