...
class PrSchedule extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'prSchedule:run {sid}';
    ...
    public function handle()
    {
        ...
        $SQL = DB::select("Select [ftp_tmpl_id],[camp_tmpl_id],[Schedule_Name],[sch_status_id],[Schedule_type],[rp_count],[ftp_tmpl_id]
               ...
               ,[rp_days],[rp_end_date],[rp_run_time],[metadata_date],[SFTP_Attachment] from [UL_RepCmp_Schedules] Where row_id = '$sch_id' AND t_type = 'P'");
        ...
        $SQLSSM = DB::select("Select * from [UL_RepCmp_Sch_status_mapping] Where sch_id = '$sch_id' AND t_type = 'P' ORDER BY row_id DESC");
...
//  $CGD = $aData[0]['asg_camp_dtls'];
        ...
        $CampSQL = DB::select("SELECT [t_id],[DCampaignID],[list_short_name],[list_level],[list_fields],[t_name],[sql],[seg_def],[seg_method],[seg_criteria],[seg_selected_criteria],[seg_grp_no],[seg_ctrl_grp_opt],[seg_camp_grp_dtls]
		    ,[seg_sample],[promoexpo_cd_opt],[promoexpo_file_opt],[promoexpo_folder],[promoexpo_file],[promoexpo_ext]
		    ,[promoexpo_ecg_opt],[promoexpo_data],[selected_fields],[selected_fields],[sql],[Report_Row],[Report_Column],[Report_Function],[Report_Sum],[Report_Show],[Chart_Image],[Report_Orientation],[Custom_SQL],[SR_Attachment],[List_Format] FROM [UP_Profile_Templates] Where row_id = '" . $CampID . "' AND t_type='P'");
        ...
        if (!empty($aData)) {
            ...
            foreach ($aData as $k => $row) {
                ...
                $CID = $aData[0]['t_id'];
                $DCampaignID = $aData[0]['DCampaignID'];
                $list_short_name = $aData[0]['list_short_name'];
                $t_name = $aData[0]['t_name'];
                $list_fields = $aData[0]['list_fields'];
                $sSQL = $aData[0]['sql'];
                ...
                $Chart_Images = $aData[0]['Chart_Image'];
        ...
        $metaData = RepCmpMetaData::where('CampaignID',$CID)->where('type','P')->first();
        if($metaData){
        ...
        if ($SchType == 'RP') {
            ...
            if ($rp_count > 1) {
                ...
                $cid1 = $cid + 1;
                DB::update("UPDATE [UP_Profile_Sequence] SET [camp_id] = " . $cid1);
            ...
            $rp_count = $rp_count + 1;
            DB::update("UPDATE [UL_RepCmp_Schedules] SET [rp_count] = $rp_count Where row_id = '$sch_id' AND t_type = 'P'");
        }
        echo 'file_Opt';
        if (trim($file_Opt) == 'Y' || trim($CD_Opt) == 'Y') {
            $date = date("m/d/y  H:i:s", time());    //Schedule stating date and time
            if ($SchType == 'RA'){
                //DB::update("UPDATE [UL_RepCmp_Status] SET [status] = 'Running', [start_time] = '" . $date . "' Where row_id = '" . $SchStatusID . "' AND t_type = 'P'");
            }elseif ($SchType == 'RP'){
                //DB::update("UPDATE [UL_RepCmp_Status] SET [status] = 'Running', [start_time] = '" . $date . "' Where row_id = '" . $SchStatusID . "' AND t_type = 'P'");
            }
            else
                //DB::update("UPDATE [UL_RepCmp_Status] SET [status] = 'Running' Where row_id = '" . $SchStatusID . "' AND t_type = 'P'");
           ...
           // $aData = collect($CampIDSQL)->map(function($x){ return (array) $x; })->toArray();
            ...
            if (trim($file_Opt) == 'Y') {
                /*if(in_array($SR_Attachment,['onlylist','both'])) {
                    ...
                    $fhead = fopen($this->filePath . 'public\\' . $folder_Name . "\\" . $this->prefix . "RPL_" . $file_Name . ".xlsx", 'P');
                    $expcols = $expcols . "\n";
                    fwrite($fhead, $expcols);
                    fclose($fhead);
                }*/
                $filename = $this->prefix.'PRF_'.$file_Name;
                if(in_array($SR_Attachment,['pdf','both'])) {
                    $PdfManage = new \PDFMerger;


                    $header = ucfirst($metaData->Category);
                    $footer = ucfirst($list_short_name);
                    $papersize = 'portrait';

                    $mMultiPageFileName = $filename;
                    $Chart_Images = json_decode($Chart_Images,true);
                    if (count($Chart_Images) > 0) {
                        $db = $this->db->getPdo();
                        $stmt = $db->prepare("EXEC dbo.sp_CRM_Profile1 ".$DCampaignID);
                        $stmt->execute();

                        foreach ($Chart_Images as $field => $Chart_Image) {

                            $sSql1 = "insert into UP_Profile_Table_temp (RowVariable, Value, TotalCount, Responder) select '".$field."', $field,  count(*) as TotalCount ,sum(case when Responder=1 then 1 else 0 end)   from  contact_view c inner join UC_Campaign_Data d on c.ds_mkc_contactid=d.ds_mkc_contactid where campaignid=$DCampaignID group by $field order by $field";
                            DB::statement($sSql1);
                            ...
                            $table = Helper::profile_datatable($dData);


                            $imgTag = '<img style="display: block;  width: 100%; height: 435px;" src="' .$Chart_Image. '" class="img-responsive">';

                            $unifilename = rand(100,10000).$filename;
                            $subheader = !empty($header) ? ' - '.str_replace( array('[',']','_') , ' '  , $field) : str_replace( array('[',']','_') , ' '  , $field);
                        ...
                        $PdfManage->merge('file', public_path('\\public\\'.$mMultiPageFileName.'.pdf'));
                    }

                }

                if(in_array($SR_Attachment,['xlsx','both'])) {
                    ...
                    foreach ($dData as $key => $ValArr){
                        ...
                        //foreach ($ValArr as $cName => $value) {
                            ...
                            $sheet->setCellValue($headerCells[0] . $j, !empty($ValArr['Value']) ? str_replace( array('[',']') , ''  , $ValArr['RowVariable'] )  : '');
                            $i++;
                            $sheet->setCellValue($headerCells[1] . $j, $ValArr['Value']);
                            $i++;
                            $sheet->setCellValue($headerCells[2] . $j, $ValArr['TotalCount']);
                            $i++;
                            $sheet->setCellValue($headerCells[3] . $j, $ValArr['Responder']);
                            $i++;
                            $sheet->setCellValue($headerCells[4] . $j, !empty($ValArr['ResponseRate']) ? number_format($ValArr['Responder']/$ValArr['TotalCount'],1) : '');
                            $i++;
                            //}
                            $j++;
                        //}
                    ...
                    $writer->save(public_path()."\\public\\".$filename.".xlsx");
                }
            }
            // open file and Write the header
            ...
            for ($i = 0; $i < $where_count; $i++) {
                ...
                /*if ((trim($file_Opt) == 'Y') && (trim($file_Ext) == "csv")) {
                    ...
                    DB::statement("insert into openrowset  ('Microsoft.ACE.OLEDB.12.0','Text;Database=" . $this->filePath .'public\\'. $folder_Name . "\\;HDR=YES;FMT=Delimited','SELECT $expcols FROM ['.$this->prefix.'RPL_$file_Name.csv]' ) $newsql");

                    $CSVTotalRec = count(file($this->filePath .'public\\'. $folder_Name . "\\".$this->prefix."RPL_$file_Name.csv", FILE_SKIP_EMPTY_LINES)) - 1;

                    if(!in_array($SR_Attachment,['onlylist','both'])){
                        unlink($this->filePath .'public\\'. $folder_Name . "\\".$this->prefix."RPL_$file_Name.csv");
                    }
                    ...
                    if(in_array($SR_Attachment,['onlylist','both'])) {
                        ...
                        $writer = new Xlsx($spreadsheet);
                        $writer->save(public_path() . '\\' . $folder_Name . "\\" . $this->prefix . "RPL_" . $file_Name . ".xlsx");
                        DB::statement("insert into OPENROWSET('Microsoft.ACE.OLEDB.12.0', 'Excel 12.0;Database=" . $this->filePath . 'public\\' . $folder_Name . "\\" . $this->prefix . "RPL_" . $file_Name . ".xlsx;','SELECT * FROM [Worksheet$]') $newsql");
                    }
                }*/
            ...
            if (trim($CD_Opt) == 'Y') {
                ...
                if (($SchType == 'RI') or ($SchType == 'RA') or (($SchType == 'RP') && ($rp_count == 2))) {
                    ...
                    $nextSQL = DB::select("SELECT [last_runtime] from [UL_RepCmp_Status] Where row_id = '" . $SchStatusID . "' AND t_type = 'P' ORDER BY row_id desc");
                ...
                echo '<br/>metadata_date';
                $metadata_date = date("Y-m-d", strtotime($metadata_date));
                DB::update("UPDATE [UL_RepCmp_Schedules] SET [metadata_date] = '$metadata_date' Where row_id = '$sch_id' AND t_type = 'P'");
                //$startDate = $date;
            ...
            if ($ftp_id != 0) {
                ...
                if ($site_type == 'FTP') {
                    ...
                    if (($conn_id) && ($login_result)) {
                        $local_file = $this->filePath .'public\\' .$folder_Name . "\\RPL_$file_Name." . "$file_Ext";
                        ...
                        ftp_close($conn_id);
                        $ftp_flag = 'Y';
                        DB::update("Update UL_RepCmp_Completed set [ftp_flag] = 'Y' Where row_id = '" . $sch_id . "' AND t_type = 'P'");
                        $smtp_flag = 'Y';
                        ...
                        $ftp_flag = 'N';
                        DB::update("Update UL_RepCmp_Completed set [ftp_flag] = 'N' Where row_id = '" . $sch_id . "' AND t_type = 'P'");
                        $smtp_flag = 'N';
                    }
                    ...
                    $fhead = fopen($this->filePath . "psftpscript_sample2.txt", 'P');
                    ...
                    if(in_array($SFTP_Attachment,['onlylist','both'])){
                        $line1 = "put ".$this->prefix."RPL_" . $file_Name . "." . $file_Ext . "\n";
                        $line3 = "mv ".$this->prefix."RPL_" . $file_Name . "." . $file_Ext . " $folder\n ";
                    }

                    if(in_array($SFTP_Attachment,['onlyreport','both']) && !empty($Report_Row)){
                        $line2 = "put ".$this->prefix."RPS_" . $file_Name . ".pdf" . "\n";
                        $line4 = "mv ".$this->prefix."RPS_" . $file_Name . ".pdf" . " $folder\n ";
                    }
                    ...
                    $fhead = fopen($this->filePath . "psftpexecution_gc_sam.bat", 'P');
                    $command = "cd/ \n";
            ...
            if ($SchType == 'RA') {
                ...
                DB::update("UPDATE [UL_RepCmp_Status] SET [status] = 'Completed',[completed_time] = '" . $date . "' ,[succ_flag] = 'Y',[file_name] = '" . $file_Name ."',total_records= '$CSVTotalRec' Where row_id = '" . $SchStatusID . "' AND t_type = 'P'");

            }
            else if ($SchType == 'RP') {
                $nextSQL = DB::select("SELECT * from [UL_RepCmp_Status] Where row_id = '" . $SchStatusID . "' AND t_type = 'P'");
                $aData = collect($nextSQL)->map(function($x){ return (array) $x; })->toArray();
                ...
                switch ($rp_run_sch) {
                    case 'daily':
                        ...
                        //echo $old_runtime.' -- ' .$next_runtime.' -- ' .$old_runtime;  die('cool');
                        break;
                    case 'weekly':
                ...
                /*    DB::insert("INSERT INTO [UL_RepCmp_Status] ([status],[last_runtime],[completed_time],[next_runtime],[succ_flag],[file_name],[t_type]) VALUES ('".$status."','" . $last_runtime . "','" . $date . "','" . $next_runtime . "','Y','" . $file_Name . "','P'");*/
                ...
                if (date("Y-m-d", strtotime($next_runtime)) < date("Y-m-d", strtotime($rp_end_date))){
                    DB::insert("INSERT INTO [UL_RepCmp_Status]([sche_name],[templ_name],[start_time]
                                   ...
                                   VALUES ('$sche_name','$templ_name','$start_time','','-','','Scheduled','" . $last_runtime . "','" . $next_runtime . "','".$file_path."','$ftp_flag','P')");

                    $SQL = DB::select("Select [row_id] from [UL_RepCmp_Status] Where sche_name = '$sche_name' AND t_type = 'P' order by row_id desc");
                ...
                /*if (date("Y-m-d", strtotime($next_runtime)) < date("Y-m-d", strtotime($rp_end_date))){
                    ...
                    DB::update("UPDATE [UL_RepCmp_Status] SET [status] = 'Completed',[last_runtime] = '" . $last_runtime . "',[completed_time] = '" . $date . "',[next_runtime] = '" . $next_runtime . "',[succ_flag] = 'Y',[file_name] = '" . $file_Name. '.' . $file_Ext  . "' Where row_id = '" . $SchStatusID . "' AND t_type = 'P'");*/

            } else if ($SchType == 'RI') {
                $command = 'schtasks /delete /tn ' . $this->schtasks_dir . '\\' . $SchName . ' /f';
                Helper::schtask_curl($command);
                echo "<br/>UPDATE [UL_RepCmp_Status] SET [status] = 'Completed',[completed_time] = '" . $date . "' ,[succ_flag] = 'Y',[file_name] = '" . $file_Name . "',total_records= '$CSVTotalRec' Where row_id = '" . $SchStatusID . "' AND t_type = 'P'";
            ...
            DB::insert("Insert into [UL_RepCmp_Completed]([sche_name],[templ_name],[start_time],[next_runtime],[completed_time]
		     ...
		     ,'Completed',[file_path],'$CID','$CSVTotalRec',t_type FROM [UL_RepCmp_Status] Where row_id = '" . $SchStatusID . "' AND t_type = 'P' ORDER BY row_id DESC");

            $sSqlSRE = DB::select("SELECT * FROM UL_RepCmp_Email WHERE camp_tmpl_id = '" . $CID . "' AND t_type='P'");
                ...
                foreach ($ToUsers as $ToUser){
                    ...
                    if($user) {
                        ...
                        if (count(Mail::failures()) > 0) {
                            ...
                            DB::update("UPDATE UL_RepCmp_Email SET Email_Status = 'Sent' WHERE camp_tmpl_id = '" . $CID . "' AND t_type='P'");
                        }
                    }
                }
            ...
            $sSqlSRE = DB::select("SELECT Shared_With_User_id FROM UL_RepCmp_Share WHERE Is_Delivered <> 1 AND camp_tmpl_id='".$CID."' AND t_type='P'");
            ...
            if(isset($records) && count($records) > 0){
                foreach ($records as $record){
                    ...
                    if($user){
                        Helper::sendShareEmail($record,$user,$this->clientname,$record['Custom_Message'],$file_Name,'P');
                    }
                }
            }
