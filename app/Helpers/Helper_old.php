...
class Helper
{
    ...
    public static function getColumns($menu_level1,$menu_level2){
        ...
        array_multisort(array_column($filter_columns, 'Filter_Order'), SORT_ASC, $filter_columns);
    ...
    public static function getFilterValues($evalColumns){
        ...
        foreach ($evalColumns as $evalColumn){
            if($evalColumn['Filter'] == 1){
                ...
                if($evalColumn['Filter_Field_Type'] == 'select'){
    ...
    public static function getFiltersCondition($filters, $section = '',$visible_columns = []){
        $txtSearch = isset($filters['searchterm']) ? $filters['searchterm'][0] : '';
        $sWhere = '';
        if(count($filters) > 0 && count($visible_columns) > 0){
            $applied = [];
            foreach ($filters as $keyColumn => $filter){
                if($keyColumn != 'searchterm'){

                    if(!in_array($keyColumn,$applied) && stripos($keyColumn,'_op_') != true && (stripos($keyColumn,'_from') != false || stripos($keyColumn,'_to') != false)) {  // date filter
                        if(stripos($keyColumn,'-') != false){
                            $sepPrefixColumn = explode('-',$keyColumn);
                            $keyColumn = $sepPrefixColumn[1];
                        }


                        if(stripos($keyColumn,'_from') != false){
                            $sepColumn = explode('_from',$keyColumn);
                            $keyColumn = $sepColumn[0];
                        }elseif (stripos($keyColumn,'_to') != false){
                            $sepColumn = explode('_to',$keyColumn);
                            $keyColumn = $sepColumn[0];
                        }

                        $key = array_search($keyColumn, array_column($visible_columns, 'Field_Name'));
                        $keyColumnFrom = $sepPrefixColumn[0].'-'.$keyColumn.'_from';
                        $keyColumnTo = $sepPrefixColumn[0].'-'.$keyColumn.'_to';

                        $dateFrom = isset($filters[$keyColumnFrom][0]) ? $filters[$keyColumnFrom][0] : '';
                        $dateTo = isset($filters[$keyColumnTo][0]) ? $filters[$keyColumnTo][0] : '';
                        $Field_Name = !is_null($visible_columns[$key]['Format']) ? $visible_columns[$key]['Format'] : $visible_columns[$key]['Field_Name'];
                        if($visible_columns[$key]['Filter_Field_Type'] == 'date'){
                            if (!empty($dateFrom) || !empty($dateTo)) {
                                if(!empty($sWhere)){
                                    $sWhere .= ' AND ';
                                }
                                if (!empty($dateFrom) && empty($dateTo)) {
                                    $sWhere .= " (isnull(".$Field_Name.",'') BETWEEN '" . $dateFrom . "' AND '" . $dateFrom . "')";
                                }else if (empty($dateFrom) && !empty($dateTo)) {
                                    $sWhere .= " (isnull(".$Field_Name.",'') BETWEEN '" . $dateTo . "' AND '" . $dateTo . "')";
                                }else if (!empty($dateFrom) && !empty($dateTo)) {
                                    $sWhere .= " (isnull(".$Field_Name.",'') BETWEEN '" . $dateFrom . "' AND '" . $dateTo . "')";
                                }
                            }
                        }
                        array_push($applied,$keyColumnFrom);
                        array_push($applied,$keyColumnTo);


                    }else if(stripos($keyColumn,'_op_') != true && stripos($keyColumn,'_Form') != true && stripos($keyColumn,'_To') != true){  // value field with operator(like amount) and none operator(DS_MKC_ContactId)
                        if(stripos($keyColumn,'-') != false){
                        ...
                        if(isset($filters[$keyColumn.'_op_']) && !in_array($keyColumn,$applied) && $visible_columns[$key]['Filter'] == 1){
                            ...
                            if($keyColumn == 'Productcat1_Des') {
                                foreach ($filter as $PC1) {
                                    $tPC1Array[] = "'" . str_replace("'", "''", $PC1) . "'";
                                }
                            ...
                            if(!in_array($visible_columns[$key]['Field_Name'],['Productcat1_Des','Productcat2_Des','Product'])){
                                $Field_Name = !is_null($visible_columns[$key]['Format']) ? $visible_columns[$key]['Format'] : $visible_columns[$key]['Field_Name'];
                                if(!empty($sWhere)){
                                    $sWhere .= ' AND ';
                                }
                    ...
                    }else if (!in_array($keyColumn,$applied) && strpos($keyColumn,'_op_') != false){  //operator field like Amount_op_
        ...
        if ($txtSearch != "") {
            ...
            foreach ($visible_columns as $key=> $visible_column){
                ...
                $sWhere .= $key == 0 ? ' (' : '';
                if($visible_column['Filter'] == 1 && !in_array($visible_column['Field_Name'],['Productcat1_Des','Productcat2_Des','Product'])) {
                    ...
                    if($visible_column['Field_Name'] == 'Productcat1_Des') {
                    ...
                    if(!empty($specWhere)){
                        $sWhere .=  '('.$specWhere.') ';
                        $sWhere .=  count($visible_columns) > ($key+1) ?  " AND " : "";;
                    }
                }
            }
        }
    ...
    public static function getFiltersSummaryDetail($sumColumns = [],$detailColumns = [],$mMetadataColumns = []){
        $sumFilters = self::getFilterValues($sumColumns);
        $detailFilters = self::getFilterValues($detailColumns);
        $metadataFilters = self::getFilterValues($mMetadataColumns);
    ...
    public static function generateLeftSideFilterHtml($Filters = [],$field_prefix = ''){
        $html = '';
        $multiselect_filters_fields = [];
        foreach($Filters as $Field_Name => $Filter) {
            $Field_ID = $Field_Name . '-filter';
            $Field_Name_before_prefix = $Field_Name;
            ...
            elseif ($Filter['Filter_Field_Type'] == 'date'){

                $html .= '<div class="row">
                    <div class="col-md-6 p-r-0">
                        <input type="text" name="' . $Field_Name . '_from" onblur="applyFilters();" onchange="applyFilters();" class="form-control form-control-sm start' . $Field_Name_before_prefix . 'Id" placeholder="Enter ' . $Field_Name_before_prefix . ' From" data-placeholder="' . $Field_Name_before_prefix . ' From">
                    </div>
                    <div class="col-md-6 p-l-0">
                        <input type="text" name="' . $Field_Name . '_to" onblur="applyFilters();" onchange="applyFilters();" class="form-control form-control-sm end' . $Field_Name_before_prefix . 'Id" placeholder="Enter ' . $Field_Name_before_prefix . ' To" data-placeholder="'.$Field_Name_before_prefix.' To">
                    </div>
                </div>';

            }
            elseif ($Filter['Filter_Field_Type'] == 'numeric'){
                ...
                $html .= '<div class="row">
                    ...
                    <div class="col-md-8 p-l-0">
                        <input type="text" name="' . $Field_Name . '" onkeyup="applyFilters();" class="form-control form-control-sm" placeholder="Enter ' . $Field_Name_before_prefix . ' " data-placeholder="' . $Field_Name_before_prefix . '">
                    </div>
                </div>';
                ...
                $html .= '<select name="' . $Field_Name . '" id="' . $Field_ID . '" class="form-control form-control-sm" multiple="multiple" data-placeholder="Select Values">';
                foreach ($Filter as $F) {
                    if ($F == $Filter['Field_Display_Name'] || $F == 'select') {
                        continue;
                    }
                    $F = !is_numeric($F) ? trim($F) : $F;
                    $html .= '<option value="'. trim($F) .'">'. trim($F) .'</option>';
                }
                $html .= '</select>';
            }
        ...
        return [
            'html' => $html,
            'multiselect_filters_fields' => $multiselect_filters_fields
        ];
    }
