    ...
    public function getReport(Request $request,Ajax $ajax){
       ...
       /* $sort = ($sort == "") ? "Order By CampaignId DESC" : $sort == 'CampaignId' ? "Order By CampaignId DESC" : "Order By $sort $dir";
        ...
        if($tabid == 20){  // Running
            ...
            $query = App\Model\ProfileTemplate::query()->with(['rpmeta','rpschedule.prschstatusmap']);
            if($User_Type != 'Full_Access') {
            ...
            $query->whereHas('rpschedule.prschstatusmap',function ($qry){
            ...
            $trQuery = App\Model\ProfileTemplate::query();
            if($User_Type != 'Full_Access') {
            ...
            $trQuery->whereHas('rpschedule.prschstatusmap',function ($qry){
                $qry->where('status','Running');
            });
            ...
            cross apply (select (charindex('^', za.meta_data, P11.Pos+1))) as P12(Pos)
                       where (za.row_id = st.camp_tmpl_id and st.sch_status_id=ss.row_id AND za.t_type = 'P' and st.t_type = 'P'
...
cross apply (select (charindex('^', za.meta_data, P11.Pos+1))) as P12(Pos)
            ...
            $tabName = 'running';
            if($rType == 'pagination'){
                $html = View::make('profile.tabs.running.table',[
                    'records' => $records,
                    'uid' => $uid,
                    'tab' => $tabName,
                ...
                $html = View::make('profile.tabs.running.index',[
                    'records' => $records,
                    'uid' => $uid,
                    'tab' => $tabName,
                    'sort_column' => $sort_column,
            ...
            $paginationhtml = View::make('profile.tabs.running.pagination-html',[
...
cross apply (select (charindex('^', za.meta_data, P11.Pos+1))) as P12(Pos)
            ...
            $nSQL = "SELECT count(*) as cnt
				  FROM UP_Profile_Templates za,UL_RepCmp_Schedules st,[UL_RepCmp_Status] ss
				   where (za.row_id = st.camp_tmpl_id and st.sch_status_id=ss.row_id AND za.t_type = 'P' and st.t_type = 'P'
            ...
            $tabName = 'scheduled';
            if($rType == 'pagination'){
                $html = View::make('profile.tabs.scheduled.table',[
                    'records' => $records,
                    'uid' => $uid,
                    'tab' => $tabName,
                    'sort_column' => $sort_column,
                ...
                $html = View::make('profile.tabs.scheduled.index',[
                    'records' => $records,
                    'uid' => $uid,
                    'tab' => $tabName,
                    'sort_column' => $sort_column,
            ...
            $paginationhtml = View::make('profile.tabs.scheduled.pagination-html',[
                'total_records' => $total_records,
                'records' => $records,
                'position' => $position,
                'records_per_page' => $records_per_page,
            ...
            $query = App\Model\ProfileTemplate::query()->with(['rpschedule.prschstatusmap']);
            if($User_Type != 'Full_Access') {
                $query->where(function ($qry) use($uid){
                    $qry->whereHas('rpshare',function ($subqry) use($uid){
                        $subqry->where('Shared_With_User_id',$uid);
...
,sc.start_time as 'StartTime',sc.completed_time as 'RunTime',
     case sc.file_name WHEN '-' THEN '-'
          else case za.t_type WHEN 'P' THEN za.promoexpo_folder+'/".$this->prefix."RPL_'+sc.file_name else za.promoexpo_folder+'/'+sc.file_name
          END END as [List],
...
from UL_RepCmp_Completed sc,UP_Profile_Templates za
where (sc.camp_id = za.t_id AND za.t_type = 'P') $uWhere";
            if (isset($_POST['obj'])) {
...
,sc.start_time as 'StartTime',sc.completed_time as 'RunTime',
     case sc.file_name WHEN '-' THEN '-'
          else case za.t_type WHEN 'P' THEN za.promoexpo_folder+'/RPL_'+sc.file_name else za.promoexpo_folder+'/'+sc.file_name
...
where (sc.camp_id = za.t_id AND za.t_type = 'P') $uWhere";
            ...
            $tabName = 'level1';
            if($rType == 'pagination'){
                $html = View::make('profile.tabs.completed.table',[
                    'records' => $records,
                    'uid' => $uid,
                    'prefix' => $this->prefix,
                    'tab' => $tabName,
                ...
                $html = View::make('profile.tabs.completed.index',[
                    'records' => $records,
                    'uid' => $uid,
                    'prefix' => $this->prefix,
                    'tab' => $tabName,
            ...
            $paginationhtml = View::make('profile.tabs.completed.pagination-html',[
                'total_records' => $total_records,
                'records' => $records,
                'position' => $position,
                'records_per_page' => $records_per_page,
            ...
            $html = View::make('profile.tabs.create.new-v1',[
                'campaigns' => $campaigns,
                'tabid' => $tabid,
                'fFields' => $fFields,
            ...
            $html = View::make('profile.tabs.create.execute', ['tabid' => $tabid])->render();
    ...
    public function getRunningTabData(Request $request)
    {
        ...
        $query = App\Model\ProfileTemplate::query()->with(['rpmeta','rpschedule.rpschstatusmap']);
        if($User_Type != 'Full_Access') {
            $query->where(function ($qry) use($uid){
                $qry->whereHas('rpshare',function ($subqry) use($uid){
                    $subqry->where('Shared_With_User_id',$uid);
    ...
    public function getCompleteTabData(Request $request)
    {
        ...
        $query = App\Model\ProfileTemplate::query()->with(['rpschedule.rpschstatusmap']);
        if($User_Type != 'Full_Access') {
            $query->where(function ($qry) use($uid){
                $qry->whereHas('rpshare',function ($subqry) use($uid){
                    $subqry->where('Shared_With_User_id',$uid);
	...
	public function reSchedule(){
        return view('profile.tabs.create.outer-schedule');
    }

    public function getList(Request $request, Ajax $ajax){
        $cSQL = DB::select("SELECT t_name From UP_Profile_Templates");
    ...
    public function getSeq(Request $request, Ajax $ajax){
        $seqSQL = DB::select('SELECT [camp_id] as cid FROM [UP_Profile_Sequence]');
        $aData= collect($seqSQL)->map(function($x){ return (array) $x; })->toArray();
        $cid = 0;
        if(!empty($aData)){
            $cid = $aData[0]['cid'];

            $arSQL = DB::select('SELECT count([t_id]) as cnt FROM [UP_Profile_Templates] WHERE t_id = '.$cid);
        ...
        $cid = $cid + 1;
        DB::update("UPDATE [UP_Profile_Sequence] SET [camp_id] = " .$cid);
        return $ajax->success()
            ->appendParam('cid',$cid)
            ->response();
    }
    ...
    public function arSchData(Request $request, Ajax $ajax){
        ...
        if ($pgaction == 'Sch_campaign1') {
            ...
            $list_fields = json_encode($params->list_fields);
            ...
            $tables = [];
            $imgPath = '';
            if(!empty($params->pdfdata) && in_array($SR_Attachment,['pdf','both'])) {

                $filename = $this->prefix.'PRF_'.$t_name;
                foreach ($params->pdfdata as $pdfdata) {
                    $img = $pdfdata->chart_img_png;
                    $table = $pdfdata->chart_table;
                    if (strpos($img, 'data:image/png;base64,') !== false) {
                        $img = str_replace('data:image/png;base64,', '', $img);
                        $img = str_replace(' ', '+', $img);
                        $data = base64_decode($img);
                        $cIpath = UPLOAD_DIR . uniqid() . '.png';

                        file_put_contents($cIpath, $data);
                        $cI[$pdfdata->row_variable] = $cIpath;
                        $imgPath = $cIpath;
                        $imgTag = '<img src="' .$cIpath. '" class="img-responsive">';
                    }
                }
            ...
            $eFile = $t_name;
            $eExt = 'xlsx';
            /*$insSQL = "INSERT INTO [UP_Profile_Templates]([t_id],[User_ID],[t_name],[t_type],[list_short_name],[list_level],[list_fields],[filter_criteria],[Customer_Exclusion_Criteria],[Customer_Inclusion_Criteria],[filter_condition],[Customer_Exclusion_Condition],[Customer_Inclusion_Condition],[selected_fields],[sql],[seg_def],[seg_noLS],[seg_method]
...
VALUES
('$CID','$uid','$CName','P','$listShortName','$list_level','$list_fields','$filterVal','$customerExclusionVal','$customerInclusionVal','$filter_condition','$Customer_Exclusion_Condition','$Customer_Inclusion_Condition','$selected_fields','$sSQL','$DFS','$noLS','$lssm'
...
,'$eFolder','$eFile','$eExt','$CGOpt','$eData','$metaStr','$rv','$cv','$fu','$sv','$sa','$is_public','$custom_sql','$ct','$cI','$as','$lv','$SR_Attachment','$list_format','$report_orientation')";
            ...
            $report = new App\Model\ProfileTemplate();
            $report->t_id                         = $CID;
            $report->DCampaignID                         = $CampaignID;
            $report->User_ID                      = $uid;
            $report->t_name                       = $CName;
            $report->t_type                       = 'P';
            ...
            $metadata->CampaignID  = $params->CID;
            ...
            if (($rtype == 'RA') || ($rtype == 'RP') || ($rtype = 'RI')) {
                ...
                $SQL = DB::select("Select [row_id],[sql] from [UP_Profile_Templates] Where t_name = '$CName' AND User_ID = '$uid' AND t_type='P'");
                $aData = collect($SQL)->map(function($x){ return (array) $x; })->toArray();
                if (!empty($aData)) {
                ...
                if ($rtype == 'RA') {
                    ...
                    DB::insert("INSERT INTO [UL_RepCmp_Schedules]([Schedule_Name]
                                     ...
                                     values ('$sName','$rtype','$RA_Dt','$RA_time','$ftp_id','$Camp_temp_id','P','$SFTP_Attachment')");


                    //Insert into UL_RepCmp_Schedules table

                    $SchtempSQL = DB::select("Select [row_id],[runat_date],[runat_time] from [UL_RepCmp_Schedules] Where Schedule_Name = '$sName' AND t_type = 'P'");
                    ...
                    // Schedule the task
                    $command = 'schtasks /create /tn ' . $this->schtasks_dir . '\\' . $sName . ' /tr "\'' . $this->phpPath . '\' -f \'' . $this->filePath . 'artisan\' prSchedule:run '.$sch_id.'  " /sc once /st ' . $RA_time . ' /sd ' . $RA_Dt . ' /ru Administrator';
                    ...
                    DB::insert("INSERT INTO [UL_RepCmp_Status]([sche_name],[templ_name],[start_time],[next_runtime]
                                       ...
                                       VALUES ('$sName','$CName','$date1','$RA_Dt $RA_time','','$dbfilename','','".$ftp_data['enable']."','Scheduled','$eFolder','','P')");
                ...
                ,[rp_end_date],[rp_run_sch],[rp_run_time],[ftp_tmpl_id],[camp_tmpl_id],[rp_count],[rp_days],[rp_months_weeks],[metadata_date],[t_type],[SFTP_Attachment])
                                ...
                                ,'$ftp_id','$Camp_temp_id',1,'$mo','$r_Str','$metadata_date','P','$SFTP_Attachment')");

                    $SchtempSQL = DB::select("Select [row_id],[rp_start_date],[rp_end_date],[rp_run_time] from [UL_RepCmp_Schedules]
                                    Where Schedule_Name = '$sName' AND t_type = 'P'");
                    ...
                    /*$fh = fopen( $this->filePath.'prSchedule.bat', 'w' );
                    fclose($fh);

                    $fhead = fopen($this->filePath."prSchedule.bat", 'P');
                    $command = "php artisan prSchedule:run ".$sch_id." \n";
                    ...
                    switch ($rp_run_sch) {
                        case 'daily':
                            $command = 'schtasks /create /tn ' . $this->schtasks_dir. '\\' . $sName . ' /tr "\'' . $this->phpPath . '\' -f \'' . $this->filePath . 'artisan\' prSchedule:run '.$sch_id.' " /sc ' . $rp_run_sch . ' /sd ' . $rp_start_date . ' /st ' . $rp_run_time . '  /ed ' . $rp_end_date_cm . '  /z /ru Administrator';

                            break;
                        case 'weekly':
                            $command = 'schtasks /create /tn ' . $this->schtasks_dir . '\\' . $sName . ' /tr "\'' . $this->phpPath . '\' -f \'' . $this->filePath . 'artisan\' prSchedule:run '.$sch_id.' " /sc ' . $rp_run_sch . ' /mo ' . $mo . ' /sd ' . $rp_start_date . ' /st ' . $rp_run_time . '  /ed ' . $rp_end_date_cm . ' /d ' . $dayStr . ' /z /ru Administrator';
                            break;
                        case 'monthly':
                            if ($dayStr == 'last')
                                $command = 'schtasks /create /tn ' . $this->schtasks_dir . '\\' . $sName . ' /tr "\'' . $this->phpPath . '\' -f \'' . $this->filePath . 'artisan\' prSchedule:run '.$sch_id.' " /sc ' . $rp_run_sch . ' /sd ' . $rp_start_date . ' /st ' . $rp_run_time . '  /ed ' . $rp_end_date . ' /m ' . $monthStr . ' /mo lastday /z /ru Administrator';
                            else
                                $command = 'schtasks /create /tn ' . $this->schtasks_dir . '\\' . $sName . ' /tr "\'' . $this->phpPath . '\' -f \'' . $this->filePath . 'artisan\' prSchedule:run '.$sch_id.' " /sc ' . $rp_run_sch . ' /sd ' . $rp_start_date . ' /st ' . $rp_run_time . '  /ed ' . $rp_end_date_cm . ' /m ' . $monthStr . ' /d ' . $dayStr . ' /z /ru Administrator';
                            break;
                    ...
                    DB::insert("INSERT INTO [UL_RepCmp_Status]([sche_name],[templ_name],[start_time],[next_runtime]
                               ...
                               VALUES ('$sName','$CName','$date1','$next_runDate $rp_run_time','','$dbfilename','','".$ftp_data['enable']."','Scheduled','$eFolder','$rp_end_date $rp_run_time','P')");
                    //Insert into UL_RepCmp_Status table
                }
                else if ($rtype == 'RI') {
                    ...
                    DB::insert("INSERT INTO [UL_RepCmp_Schedules]([Schedule_Name],[Schedule_type],[ftp_tmpl_id],[camp_tmpl_id],[t_type],[SFTP_Attachment])
                                VALUES ('$sName','$rtype','$ftp_id','$Camp_temp_id','P','$SFTP_Attachment')");

                    $SchtempSQL = DB::select("Select [row_id] from [UL_RepCmp_Schedules] Where Schedule_Name COLLATE Latin1_General_CS_AS = '$sName' AND t_type = 'P'");
                    ...
                    $command = 'schtasks /create /tn ' . $this->schtasks_dir . '\\' . $sName . ' /tr "\'' . $this->phpPath . '\' -f \'' . $this->filePath . 'artisan\' prSchedule:run '.$sch_id.'  " /sc once /st ' . $time . ' /sd ' . $date . ' /ru Administrator';
                    Helper::schtask_curl($command);
                    ...
                    DB::insert("INSERT INTO [UL_RepCmp_Status]([sche_name],[templ_name],[start_time]
                                   ...
                                   VALUES ('$sName','$CName','$date1','','$dbfilename','','Running','$eFolder','".$ftp_data['enable']."','P')");
                }  //if($rtype == 'RI')

                //Get Schdule row_id to update the status
                //   if($rtype == 'RI')
                $SQL = DB::select("Select [row_id] from [UL_RepCmp_Status] Where sche_name = '$sName' AND t_type = 'P'");
                /*  else
                ...
                //DB::update("Update UL_RepCmp_Schedules set [sch_status_id] = '" . $Sch_row_id . "' Where row_id = '" . $sch_id . "' AND t_type = 'P'");
                //Status update to Scheduled

                DB::insert("INSERT INTO [UL_RepCmp_Sch_status_mapping] ([sch_id],[sch_status_id],[t_type]) VALUES ('".$sch_id."','".$Sch_row_id."', 'P')");
            ...
            if ($SMTPArray[0] == 'Y') {
                ...
                DB::update("Update UL_RepCmp_Schedules set [smtp_flag]= '" . $smtp_flag . "',[semail_to] = '" . $SMTPArray[3] . "',[semail_cc] = '" . $SMTPArray[4] . "',[semail_bcc] = '" . $SMTPArray[5] . "',[semail_sub] = '" . $SMTPArray[6] . "',[semail_comments] = '" . $SMTPArray[7] . "'
                                     ,[femail_to] = '" . $SMTPArray[8] . "',[femail_cc] = '" . $SMTPArray[9] . "',[femail_bcc] = '" . $SMTPArray[10] . "',[femail_sub] = '" . $SMTPArray[11] . "',[femail_comments] = '" . $SMTPArray[12] . "' Where row_id = '" . $sch_id . "' AND t_type = 'P'");
            }
            ...
            if (isset($SREmailData['enable']) && $SREmailData['enable'] == 'Y') {
                ...
                DB::insert("INSERT INTO UL_RepCmp_Email ([User_id],[Email_Flag],[camp_tmpl_id],[remail_to],[remail_cc],[remail_bcc],[remail_sub],[remail_comments],[t_type],[Email_Status],[Email_Attachment]) values ('" . Auth::user()->User_ID . "','" . $SREmailData['enable'] . "','" . $CID . "','" . $SREmailData['to'] . "','" . $SREmailData['cc'] . "','" . $SREmailData['bcc'] . "','" . $SREmailData['sub'] . "','" . $SREmailData['message'] . "','P','pending','" . $SREmailData['Email_Attachment']. "')");
            }
           ...
           // $ShareArray = explode(":", $ShareStr);
            if (isset($ShareData['enable']) && $ShareData['enable'] == 'Y') {
                ...
                Helper::shareReport($CID,'P',$user_id,$users,$limitedtextarea4,$this->clientname,0,0);
            }
            if(!empty($list_fields)){
                if( strpos($list_fields, ',') !== false ) {
                    $fields = explode(',',$list_fields);
                }else{
                    $fields[] = $list_fields;
                }
                ...
                //Helper::generateProfilePDF($fields,$cv,ucfirst($fu),$sv,$sa,$sSQL,$list_level,$listShortName,$imgTag,$imgPath,$eFolder,$file_name,$this->prefix . 'RPS_',$SR_Attachment,$rpDesc,$report_orientation);
            }
         }
            ...
            $sSqlCheck = DB::select("SELECT * FROM [UP_Profile_Templates] WHERE t_id = '$CID' AND t_type='P'");
            ...
            DB::update("UPDATE [UP_Profile_Sequence] SET [camp_id] = " .$campaign_id);
            ...
            DB::insert("INSERT INTO [UP_Profile_Templates] ([t_id],[User_ID],[DCampaignID], [t_name],[t_type],[list_short_name],[list_level],[list_fields],[filter_criteria],
		  ...
		  '$t_name' as [t_name],'P' as [t_type],[list_short_name],[list_level],[list_fields],[filter_criteria],
		  [Customer_Exclusion_Criteria],[Customer_Inclusion_Criteria],[filter_condition],[Customer_Exclusion_Condition],
		  [Customer_Inclusion_Condition],[selected_fields],[sql],[seg_def],[seg_noLS],[seg_method],[seg_criteria],
		  [seg_selected_criteria],[seg_grp_no],[seg_ctrl_grp_opt],[seg_camp_grp_dtls],[seg_camp_grp_proportion],
		  [seg_camp_grp_sel_cri],[seg_sample],[promoexpo_cd_opt],[promoexpo_file_opt],[promoexpo_folder],'$t_name' as [promoexpo_file],
		  [promoexpo_ext],[promoexpo_ecg_opt],[promoexpo_data],
		  [Report_Row],[Report_Column],[Report_Function],[Report_Sum],[Report_Show],[is_public],[Custom_SQL],[Chart_Type],[Chart_Image] as [Chart_Image],[Axis_Scale],[Label_Value], [SR_Attachment],[List_Format],[Report_Orientation] FROM [UP_Profile_Templates] 
		  WHERE  t_id = '$CID' AND t_type='P'");
            ...
            if (($rtype == 'RA') || ($rtype == 'RP') || ($rtype = 'RI')) {
                ...
                // Common to Both
                $SQL = DB::select("Select [row_id],[list_short_name],[sql] from [UP_Profile_Templates] Where t_name = '$CName' AND User_ID = '$uid' AND t_type='P'");
                $aData = collect($SQL)->map(function($x){ return (array) $x; })->toArray();
                if (!empty($aData)) {
                    $Camp_temp_id = $aData[0]['row_id'];
                    $sSQL = $aData[0]['sql'];
                ...
                if ($rtype == 'RA') {
                    ...
                    DB::insert("INSERT INTO [UL_RepCmp_Status]([sche_name],[templ_name],[start_time],[next_runtime]
                                       ,[completed_time],[file_name],[succ_flag],[ftp_flag],[status],[file_path],[last_runtime],[t_type])
                                       VALUES ('$sName','$CName','$date1','$RA_Dt $RA_time','','$file_name','','".$ftp_data['enable']."','Scheduled','$eFolder','','P')");
                    //Insert into UL_RepCmp_Status table
                ...
                ,[rp_end_date],[rp_run_sch],[rp_run_time],[ftp_tmpl_id],[camp_tmpl_id],[rp_count],[rp_days],[rp_months_weeks],[metadata_date],[t_type],[SFTP_Attachment])
                    ...
                    DB::insert("INSERT INTO [UL_RepCmp_Status]([sche_name],[templ_name],[start_time],[next_runtime]
                               ,[completed_time],[file_name],[succ_flag],[ftp_flag],[status],[file_path],[run_until],[t_type])
                               VALUES ('$sName','$CName','$date1','$next_runDate $rp_run_time','','$file_name','','".$ftp_data['enable']."','Scheduled','$eFolder','$rp_end_date $rp_run_time','P')");
                    //Insert into UL_RepCmp_Status table
                    ...
                    DB::insert("INSERT INTO [UL_RepCmp_Status]([sche_name],[templ_name],[start_time]
                                   ,[completed_time],[file_name],[succ_flag],[status],[file_path],[ftp_flag],[t_type])
                                   VALUES ('$sName','$CName','$date1','','$file_name','','Running','$eFolder','".$ftp_data['enable']."','P')");
                }  //if($rtype == 'RI')
    ...
    public function updateProfile(Request $request, Ajax $ajax){
        ...
        $profile = App\Model\ProfileTemplate::where('t_id',$t_id)->first();
        ...
        $tables = [];
        $imgPath = '';
        if(!empty($pdfdatas) && in_array($profile->SR_Attachment,['pdf','both'])) {

            $filename = $this->prefix.'PRF_'.$profile->t_name;
...
md.SKU,md.Coupon,md.Sort_Column,md.Sort_Order FROM [UP_Profile_Templates] rt INNER JOIN [UL_RepCmp_MetaData] md ON rt.t_id = md.CampaignID AND md.type = 'P' AND
 rt.row_id = '$tempid'");
        $aData= collect($sSQL)->map(function($x){ return (array) $x; })->toArray();
        $aData = $aData[0];*/

        $record = App\Model\ProfileTemplate::with(['rpmeta'])->where('row_id',$tempid)->first();
        if(!$record){
            return $ajax->fail()
                ->jscallback()
    ...
    public function generateSummaryReport(Request $request,Ajax $ajax){
        ...
        $is_save = $request->input('save');

        /*
         * Sp1 – Onetime - sp_CRM_Profile1 -
         * Sp2 – sp_CRM_Profile2 - in Loop for each variable selected
         * */

        $db = $this->db->getPdo();
        $stmt = $db->prepare("EXEC dbo.sp_CRM_Profile1 ".$campaign_id);
        $stmt->execute();
        $response = [];
        foreach ($fields as $field){

            $sSql1 = "insert into UP_Profile_Table_temp (RowVariable, Value, TotalCount, Responder) select '".$field."', $field,  count(*) as TotalCount ,sum(case when Responder=1 then 1 else 0 end)   from  contact_view c inner join UC_Campaign_Data d on c.ds_mkc_contactid=d.ds_mkc_contactid where campaignid=$campaign_id group by $field order by $field";
            DB::statement($sSql1);
            $sSql2 = "update up_Profile_table_temp set ResponseRate = 100*cast(Responder as int)/cast(totalcount as int) where rowvariable='".$field."'";
            DB::update($sSql2);
            $sSql3 = "insert into UP_Profile_Table_temp (RowVariable) values ('".$field."')";
            DB::insert($sSql3);

            $sSql4 = "select * from up_Profile_table_temp WHERE RowVariable = '".$field."' order by RowVariable";
            ...
            $result = Helper::profile_datatable($dData);
            $response[] = [
                'row_variable' => $field,
                'column_variable' => 'Responder',
                'function_variable' => 'count',
                'show_as' => 'number',
                'sum_variable' => '',
                'chart_variable' => 'column',
                'chart_axis_scale' => 'lin',
                'chart_label_value' => '0',
        ...
        $sSql4 = "select * from up_Profile_table_temp order by RowVariable";
        $dData = DB::select($sSql4);
        //$dData = DB::select("EXEC sp_CRM_Profile2b '".$field."'");
        $dData = collect($dData)->map(function($x){ return (array) $x; })->toArray();

        return $ajax->success()
            ->appendParam('save',$is_save)
            ->appendParam('data',$dData)
            ->appendParam('response',$response)
            ->jscallback('ajax_run_profile_result')
            ->response();
    }

    public function generateXLSX_new(Request $request,Ajax $ajax){
        $profileXLSXdata = json_decode($request->input('profileXLSXdata'),true);
        $filename = $this->prefix.'Profile_'.$request->input('filename');
        $headerCells = config('constant.XlsxHeaderCells');
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();

        $headings = ['RowVariable', 'TotalCount', 'Responder', 'ResponseRate'];
        $j = 2;
        foreach ($profileXLSXdata as $ValArr){

            foreach ($ValArr as  $head) {
                //$sheet->setCellValue($headerCells[0] .'1', str_replace( array('[',']') , ''  , $head['RowVariable']));
                $sheet->setCellValue($headerCells[0] .'1', 'RowVariable');
                $sheet->setCellValue($headerCells[1] .'1', 'TotalCount');
                $sheet->setCellValue($headerCells[2] .'1', 'Responder');
                $sheet->setCellValue($headerCells[3] .'1', 'ResponseRate');
            }

            foreach ($ValArr as $cName => $value) {
                $sheet->setCellValue($headerCells[0] . $j, $value['Value']);
                $sheet->setCellValue($headerCells[1] . $j, $value['TotalCount']);
                $sheet->setCellValue($headerCells[2] . $j, $value['Responder']);
                $sheet->setCellValue($headerCells[3] . $j, !empty($value['ResponseRate']) ? number_format($value['Responder']/$value['TotalCount'],1).'%' : '');
                $j++;
            }
        }

        /*$j = 2;
        foreach ($profileXLSXdata as $key => $ValArr){
            $i = 0;
            foreach ($ValArr as $cName => $value) {
                $i++;
                $sheet->setCellValue($headerCells[0] . $j, $value['Value']);
    ...
    public function generateXLSX(Request $request,Ajax $ajax){
        ...
        $headings = ['RowVariable', 'Value', 'Universe', 'Responder', 'Response Rate'];
        foreach ($profileXLSXdata as $colArr){
            $i = 0;
            foreach ($headings as  $head) {
                $sheet->setCellValue($headerCells[$i].'1', $head);
                $i++;
            }
            break;
        }
        ...
        foreach ($profileXLSXdata as $key => $ValArr){
            $i = 0;
            foreach ($ValArr as $cName => $value) {
                //echo $headerCells[$i] . $j."--".$value['RowVariable']."--".$value['Value']."---".$value['TotalCount']."---".$value['Responder']."---".$value['ResponseRate']."<br>";
                //echo $headerCells[0] . $j."--".$value['RowVariable']."--".$value['Value']."---".$value['TotalCount']."---".$value['Responder']."---".$value['ResponseRate']."<br>";
                //if(!in_array($cName,$notAllowed)) {
                $sheet->setCellValue($headerCells[0] . $j, !empty($value['Value']) ? str_replace( array('[',']') , ''  , $value['RowVariable'] )  : '');
                $i++;
                $sheet->setCellValue($headerCells[1] . $j, $value['Value']);
                $i++;
                $sheet->setCellValue($headerCells[2] . $j, $value['TotalCount']);
                $i++;
                $sheet->setCellValue($headerCells[3] . $j, $value['Responder']);
                $i++;
                $sheet->setCellValue($headerCells[4] . $j, !empty($value['ResponseRate']) ? number_format($value['Responder']/$value['TotalCount'],1) : '');
                $i++;
                //}
                $j++;
            }
    ...
    public function generatePDF(Request $request,Ajax $ajax){
        ...
        foreach ($pdfdatas as $pdfdata){
            ...
            echo $html;*/
            PDF::loadView('layouts.profile-pdf', [
