@php
    $rpstatus = !empty($record['rpschedule']['ccschstatusmap'][0]) ? $record['rpschedule']['ccschstatusmap'] : [];
...
<tr>
    ...
    <td>@if($record['rpschedule']['Schedule_type'] == 'RP')
            {{ ucfirst($record['rpschedule']['rp_run_sch']) }}
        @elseif($record['rpschedule']['Schedule_type'] == 'RA')
            Once
        @else
            Once
        @endif
    ...
    <td class="text-center">
        <select  onchange='show_Create_library($(this))' class='form-control-sm' style="border-color: #bfe6f6;text-align-last: center;">
            ...
            <option value='view,{!! $record['row_id'] !!},"{!! $record['Scored_File_Name'] !!}"'>View</option>
            <option value='delete,{!! $record['row_id'] !!},"{!! $record['Scored_File_Name'] !!}"'>Delete</option>
        </select>
    </td>
</tr>
