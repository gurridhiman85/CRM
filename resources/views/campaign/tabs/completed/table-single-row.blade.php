...
/*    $sSql = "SELECT count(*) as cnt FROM UL_RepCmp_Share WHERE User_id = '".$uid."' AND camp_tmpl_id = '".$record->row_id."' AND t_type = 'A'";
    ...
    */?>
    ...
    <td class="text-center">
        <?php
        $ListXLSX = $record->promoexpo_folder.'\\'.$prefix.'CAL_'.$record->promoexpo_file.substr($record->t_name, -14).'.'.$record->promoexpo_ext;
        ?>
    ...
    <td class="text-center pl-0 pt-1">
        @php
            $ListPDF = $record->promoexpo_folder.'\\'.$prefix.'CAL_'.$record->promoexpo_file.substr($record->t_name, -14).'.pdf';
        @endphp
    ...
    <td class="text-center">
        @php
            $SummaryXLSX = $record->promoexpo_folder.'\\'.$prefix.'CAM_'.$record->promoexpo_file.substr($record->t_name, -14).'.xlsx';
        @endphp
    ...
    <td class="text-center pl-0 pt-1">
        @php
            $SummaryPDF = $record->promoexpo_folder.'\\'.$prefix.'CAM_'.$record->promoexpo_file.substr($record->t_name, -14).'.pdf';
        @endphp
   ...
   {{-- <td class="text-center">
    ...
    <td class="text-center">
        <select  onchange='show_Create_library($(this))' class='form-control-sm' style="border-color: #bfe6f6;text-align-last: center;">
            ...
            <option value='runcamp,{!! $record->row_id !!},"{!! $record->list_short_name !!}"'>Run Campaign</option>
