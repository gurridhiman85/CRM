<?php
$rpschstatusmaps = isset($record['rpschedule']['rpschstatusmap']) ? $record['rpschedule']['rpschstatusmap'] : [];
foreach ($rpschstatusmaps as $rpschstatusmap){
    if($rpschstatusmap['status'] == 'Completed') continue;
   // echo '<pre>'; print_r($rpschstatusmap); die;
$start_time = !empty($rpschstatusmap['start_time']) ? $rpschstatusmap['start_time'] : date('Y-m-d h:i');
$completed_time = !empty($rpschstatusmap['completed_time']) ? $rpschstatusmap['completed_time'] : date('Y-m-d h:i');
//$rpschstatusmap = isset($record->rpschedule->rpschstatusmap) ? $record->rpschedule->rpschstatusmap : [];
...
<tr>
    ...
    <td>{!! !empty($rpschstatusmap['file_name']) ? $rpschstatusmap['file_name'] : $record->list_short_name !!}</td>
    <td>
        @if(!empty($category))
    ...
    <td class="text-center">{!! !empty($rpschstatusmap['ftp_flag']) ? $rpschstatusmap['ftp_flag'] : 'N' !!}</td>
...
/*    $sSql = "SELECT count(*) as cnt FROM UL_RepCmp_Share WHERE User_id = '".$uid."' AND camp_tmpl_id = '".$record->row_id."' AND t_type = 'A'";
    ...
    */?>
    ...
    <td class="text-center">{!! !empty($rpschstatusmap['total_records']) ? number_format($rpschstatusmap['total_records']) : 0 !!}</td>
    <td class="text-center">
        <?php

        $ListXLSX = $record->promoexpo_folder.'\\'.$prefix.'RPL_'.$rpschstatusmap['file_name'].'.xlsx';
        ?>
    ...
    <td class="text-center pl-0 pt-1">
        @php
            $ListPDF = $record->promoexpo_folder.'\\'.$prefix.'RPL_'.$rpschstatusmap['file_name'].'.pdf';
        @endphp
    ...
    <td class="text-center">
        @php
            $SummaryXLSX = $record->promoexpo_folder.'\\'.$prefix.'RPS_'.$rpschstatusmap['file_name'].'.xlsx';
        @endphp
    ...
    <td class="text-center pl-0 pt-1">
        @php
            $SummaryPDF = $record->promoexpo_folder.'\\'.$prefix.'RPS_'.$rpschstatusmap['file_name'].'.pdf';
        @endphp
   ...
   {{-- <td class="text-center">
    ...
    <td class="text-center">
        <select  onchange='show_Create_library($(this))' class='form-control-sm' style="border-color: #bfe6f6;text-align-last: center;">
            ...
            <option value='deleteolderversion,{!! $rpschstatusmap['row_id'] !!},"{!! $record->list_short_name !!}",{!! $record->t_id !!}'>Delete</option>
        </select>
    </td>
</tr>
<?php } ?>
