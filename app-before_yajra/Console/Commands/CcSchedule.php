...
class CcSchedule extends Command
{
    ...
    public function handle()
    {
        ...
        $CampSQL = DB::select("SELECT [t_id],[list_short_name],[list_level],[t_name],[sql],[seg_def],[seg_method],[seg_criteria],[seg_selected_criteria],[seg_grp_no],[seg_ctrl_grp_opt],[seg_camp_grp_dtls]
		    ...
		    ,[promoexpo_ecg_opt],[promoexpo_data],[selected_fields],[selected_fields],[sql],[Report_Row],[Report_Column],[Report_Function],[Report_Sum],[Report_Show],[Report_Orientation],[Custom_SQL],[SR_Attachment],[List_Format] 
        ...
        if ((trim($file_Opt) == 'Y' && trim($sSQL) != '') || ((trim($CD_Opt) == 'Y') && trim($sSQL) != '')) {
            ...
            for ($i = 0; $i < $where_count; $i++) {
                ...
                if ((trim($file_Opt) == 'Y') && (trim($file_Ext) == "csv")) {
                    ...
                    DB::statement("insert into openrowset  ('Microsoft.ACE.OLEDB.12.0','Text;Database=" . $this->filePath .'public\\'. $folder_Name . "\\;HDR=YES;FMT=Delimited','SELECT $expcols FROM [".$this->prefix."CAL_$file_Name.csv]' ) $newsql");
                    //var_dump($aData);
