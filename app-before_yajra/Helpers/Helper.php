...
class Helper
{
    ...
    public static function getLookupFiltersFieldsValues($params = [
		...
		$campaigns = $countries = $ZSS_Segments = $MemberSegments = $AddressQualities = $DonorSegments = $EventSegments =$LifecycleSegments = $ActivityCat1 = $ActivityCat2 = $Activity = [];
        ...
        if($params['ActivityCat1']) {
            $ActivityCat1 = DB::table('Sales_View')
                ->distinct()
                ->whereNotNull('ActivityCat1')
                ->where('ActivityCat1', '<>', '')
                ->orderBy('ActivityCat1')
                ->pluck('ActivityCat1')->toArray();
        }

        if($params['ActivityCat2']) {
            $ActivityCat2 = DB::table('Sales_View')
                ->distinct()
                ->whereNotNull('ActivityCat2')
                ->where('ActivityCat2', '<>', '')
                ->orderBy('ActivityCat2')
                ->pluck('ActivityCat2')->toArray();
        }

        if($params['Activity']) {
            $Activity = DB::table('Sales_View')
                ->distinct()
                ->whereNotNull('Activity')
                ->where('Activity', '<>', '')
                ->orderBy('Activity')
                ->pluck('Activity')->toArray();
        }
        return [
            'campaigns' => $campaigns,
            'countries' => $countries,
            ...
            'ActivityCat1' => $ActivityCat1,
            'ActivityCat2' => $ActivityCat2,
            'Activity' => $Activity,
        ];
    }
