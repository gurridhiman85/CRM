...
class SendCampaignEmail extends Mailable
{
    ...
    public function build()
    {
        try{
            ...
            $is_listXLSX_exist = file_exists(public_path('\\'.$folder).'\\'.$prefix.'CAL_'.$file_Name.'.xlsx');
            $is_listPDF_exist = file_exists(public_path('\\'.$folder).'\\'.$prefix.'CAL_'.$file_Name.'.pdf');

            $Report_Row = $this->recData->data->Report_Row;
            $is_summaryXLSX_exist = file_exists(public_path('\\'.$folder).'\\'.$prefix.'CAM_'.$file_Name.'.xlsx');
            $is_summaryPDF_exist = file_exists(public_path('\\'.$folder).'\\'.$prefix.'CAM_'.$file_Name.'.pdf');
            ...
            if($is_listXLSX_exist && in_array($EmailAttachment,['onlylist','both'])){
                $mail = $mail->attach(public_path('\\'.$folder).'\\'.$prefix.'CAL_'.$file_Name.'.xlsx', [
                    'as' => $prefix.'CAL_'.$file_Name.'.xlsx',
                    'mime' => 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                ]);
            }

            if($is_listPDF_exist && in_array($EmailAttachment,['onlylist','both'])){
                $mail = $mail->attach(public_path('\\'.$folder).'\\'.$prefix.'CAL_'.$file_Name.'.pdf', [
                    'as' => $prefix.'CAL_'.$file_Name.'.pdf',
                    'mime' => 'application/pdf',
                ]);
            }

            if(!empty($Report_Row) && $is_summaryXLSX_exist && in_array($EmailAttachment,['onlyreport','both'])){
                $mail = $mail->attach(public_path('\\'.$folder).'\\'.$prefix.'CAM_'.$tname.'.xlsx', [
                    'as' => $prefix.'CAM_'.$tname.'.xlsx',
                    'mime' => 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                ]);
            }
            if(!empty($Report_Row) && $is_summaryPDF_exist && in_array($EmailAttachment,['onlyreport','both'])){
                $mail = $mail->attach(public_path('\\'.$folder).'\\'.$prefix.'CAM_'.$file_Name.'.pdf', [
                    'as' => $prefix.'CAM_'.$file_Name.'.pdf',
                    'mime' => 'application/pdf',
                ]);
            }
