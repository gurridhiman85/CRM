...
cross apply (select (charindex('^', za.meta_data, P11.Pos+1))) as P12(Pos)
            ...
            $tabName = 'running';
            if($rType == 'pagination'){
                $html = View::make('campaign.tabs.running.table',['records' => $records,'uid' => $uid,'tab' => $tabName])->render();
            }else{
                $html = View::make('campaign.tabs.running.index',['records' => $records,'uid' => $uid,'tab' => $tabName])->render();
            }
...
where (sc.camp_id = za.t_id AND za.t_type = 'C') $uWhere";
            ...
            $tabName = 'level1';
            if($rType == 'pagination'){
                $html = View::make('campaign.tabs.level1.table',['records' => $records,'uid' => $uid,'prefix' => $this->prefix,'tab' => $tabName])->render();
            }else{
                $html = View::make('campaign.tabs.level1.index',['records' => $records,'uid' => $uid,'prefix' => $this->prefix,'tab' => $tabName])->render();
            }
    ...
    public function ccSchData(Request $request, Ajax $ajax){
		...
		VALUES
           ...
           // $lastPart = explode($listShortName,$CName);
            ...
            Helper::generateSrPDF($rv,$cv,ucfirst($fu),$sv,$sa,$sSQL,$list_level,$listShortName,$imgTag,$imgPath,$eFolder,$file_name,$this->prefix.'CAM_',$SR_Attachment,$rpDesc,$report_orientation);
        }
		  ...
		  WHERE  t_id = '$CID' AND t_type='C'");
            ...
            Helper::generateSrPDF($rv,$cv,$fu,$sv,$sa,$sSQL,$list_level,$list_short_name,$imgTag,$imgPath,$eFolder,$file_name,$this->prefix  . 'CAM_',$SR_Attachment,$rpDesc,$report_orientation);
        }
    }
