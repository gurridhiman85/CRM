...
class ImportzoomController extends Controller {
    ...
    public function importStep1(Request $request, Ajax $ajax) {
        ...
        //Step 2
        $sStep2TableRows = DB::select("select  s.rowid, s.customer_s2, x.dflname, s.email  from contact_temp s inner join contact x on s.customer_s2=x.dharmaname or s.customer_s2=x.firstname + ' ' + x.lastname where isnull(s.customer_s2,'')  <> '' and isnull(s.ds_mkc_contactid,'') =''
group by  s.rowid, s.customer_s2, x.dflname, x.online_attendee , s.email
