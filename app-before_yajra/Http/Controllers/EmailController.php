...
class EmailController extends Controller
{
    ...
    public function getEmails(Request $request,Ajax $ajax){
        ...
        $position = ($page-1) * $records_per_page;
            ...
            //if($zip!=""){$sWhere .= "zip like '$zip%'";}

            //$sWhere = ($sWhere != "") ? "where $sWhere" : "";
            //$sWhere1 = "Where ROWNUMBER > 0 and ROWNUMBER <= 25 ";
            $sort = ($sort == "") ? "Order by CampaignId DESC" : $sort == 'CampaignId' ? "Order by CampaignId DESC" : "Order by $sort $dir";
            //$sSQL = "SELECT [CampaignId],[CampaignName],[template],cast(Time1 as varchar(5)) as Time1,cast([StartDate] as date) as StartDate,cast([EndDate] as date) as EndDate,[subject1],[subject2],[subject3],[TestSubject],[CampaignID_Data],[TestSubjectPct],[SubjectWin] FROM (SELECT ROW_NUMBER() over ($sort) as ROWNUMBER,* from emails_configuration $sWhere) _myResults  $sWhere1";
        ...
        if($tabid == 20){
            ...
            $tabName = 'Completed';
            if($rType == 'pagination'){
                $html = View::make('email.tabs.level1.table',['records' => $records,'tab' => $tabName])->render();
            }else{
                $html = View::make('email.tabs.level1.index',['records' => $records,'tab' => $tabName])->render();
            }
