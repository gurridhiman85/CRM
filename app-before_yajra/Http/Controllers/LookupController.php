...
class LookupController extends Controller
{
    ...
    public function index(){
        ...
        return view('lookup.index',[
            ...
            /*'ActivityCat1'      =>  ['Contribution','Membership','Kessei'],
            'ActivityCat2'      =>  ['Individual'],
            'Activity'          =>  ['Contribution/Individual'],*/
        ]);
    }
    ...
    public function getFirstScreen(Request $request,Ajax $ajax){
        ...
        if($tabid == 20){
            ...
            $sSql = "SELECT * from (select *, row_number () over (partition by rown   $sort  ) as ROWNUMBER FROM   (SELECT ROW_NUMBER() over (partition by DS_MKC_ContactID $sort) as ROWN,DS_MKC_ContactID,DS_MKC_HouseholdID,Email,EmailSegment,email2,phone,dqcode_phone,Extendedname, Company,JobTitle,Address,City,State,Zip,dqcode_address,Salutation,DharmaName,Firstname,Middlename,Lastname,Suffix,Salutation2,DharmaName2,FirstName2,Middlename2,Lastname2,Suffix2,Life2date_SpendAmt,isnull(tag,0) as tag,update_date,TouchDate from Contact_View ".$whereClause['finalClause'].") _myResults where ROWN = 1 ) as a  $sWhere1 $sort";
    ...
    public function getFirstScreenData(Request $request)
    {
        ...
        $sSql = "SELECT * from (select *, row_number () over (partition by rown   $sort  ) as ROWNUMBER FROM   (SELECT ROW_NUMBER() over (partition by DS_MKC_ContactID $sort) as ROWN,DS_MKC_ContactID,DS_MKC_HouseholdID,Email,EmailSegment,email2,phone,dqcode_phone,Extendedname, Company,JobTitle,Address,City,State,Zip,dqcode_address,Salutation,DharmaName,Firstname,Middlename,Lastname,Suffix,Salutation2,DharmaName2,FirstName2,Middlename2,Lastname2,Suffix2,Life2date_SpendAmt,isnull(tag,0) as tag,update_date,TouchDate from Contact_View ".$finalClause.") _myResults where ROWN = 1 ) as a   $sort";
        //dd($sSql);
    ...
    public function downloadReport(Request $request,Ajax $ajax){
        ...
        if($screen == 'first'){
            ...
            $sort = ($sort == "" || $sort == "select") ? "Order by Updated  DESC " : "Order by $sort $dir";
...
Life2date_NZoom_Ot,	Last_5Yrs_NZoom_Ot, Last_6Mth_NZoom_Ot,	CurrentYr_NZoom_Ot,	Prior_Yr1_NZoom_Ot,	Prior_Yr2_NZoom_Ot,	Prior_Yr3_NZoom_Ot,	Prior_Yr4_NZoom_Ot
 ...
 FROM Contact_View where DS_MKC_ContactID = '$contactid'";
     ...
     $where  sa.DS_MKC_ContactID = ac.DS_MKC_ContactID AND ac.DS_MKC_ContactID = $contactid) _myResults";
            ...
            $sSQL = "SELECT * FROM (SELECT ROW_NUMBER() over ($sort) as ROWNUMBER,ac.DS_MKC_ContactID,ac.dflname,t.TouchCampaign,t.TouchStatus,t.TouchChannel,t.TouchDate,t.TouchNotes from Touch  t INNER JOIN Contact_View sa ON
     $where t.DS_MKC_ContactID = sa.DS_MKC_ContactID AND sa.DS_MKC_ContactID = $contactid) _myResults";
...
Life2date_NZoom_Ot,	Last_5Yrs_NZoom_Ot, Last_6Mth_NZoom_Ot,	CurrentYr_NZoom_Ot,	Prior_Yr1_NZoom_Ot,	Prior_Yr2_NZoom_Ot,	Prior_Yr3_NZoom_Ot,	Prior_Yr4_NZoom_Ot
    ...
    public static function Apply2ndScreenFiltersCondition($filters){
        ...
        if (!empty($txtActivityCat1)) {
            $specWhere .= " sa.ds_mkc_contactid in (select distinct ds_mkc_contactid from Sales_View where activitycat1='" . trim($txtActivityCat1) . "') ";
            $aAd++;
        }
        if (!empty($txtActivityCat2)) {
            $specWhere .= $aAd > 0 ? " and " : "";
            $specWhere .= " sa.ds_mkc_contactid in (select distinct ds_mkc_contactid from Sales_View where activitycat2= '" . trim($txtActivityCat2) . "') ";
        }
